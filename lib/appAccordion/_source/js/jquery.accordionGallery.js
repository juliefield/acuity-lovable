...
(function($) {
	...
	$.accordionGallery.checkSlideshow2 = function(){//called after prettyphoto detail close (public function)
		detailOpened=false;
		if(slideshowOn){
			controls_play.css('display', 'none');
			controls_pause.css('display', 'block');
	...
	$.accordionGallery.toggleSlideshow = function(state) {
		if(!componentInited) return;
		if(state == undefined){
			toggleSlideshow();
		}else{
	...
	$.accordionGallery.toggleDirection = function(dir) {
		if(!componentInited) return;
		if(!dir || !isEmpty(dir)){
			if(slideshowDirection == 'forward'){
	...
	$.accordionGallery.setSlideshowDelay = function(num) {
		if(!componentInited) return;
		slideshowTimeout = num;
	}
	
	$.accordionGallery.nextMedia = function() {
		if(!componentInited) return;
		if(slideshowDirection == 'forward'){
			nextSlide();
		}else{
	...
	$.accordionGallery.previousMedia = function() {
		if(!componentInited) return;
		previousSlide();
	}
	
	$.accordionGallery.openMedia = function(num) {
		if(!componentInited) return;
		if(num<0)num=0;
