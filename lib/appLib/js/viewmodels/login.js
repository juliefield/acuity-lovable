...
function LoginViewModel(o) {
    ...
    doLogin = function () {
        var product = "Acuity";
        if (a$.validateform({ formid: 'loginForm', errdiv: '.login-validation' })) {

            a$.ajax({ type: "GET", async: false, data: { cmd: "loginkey", username: self.newloginusername(), product: product }, dataType: "json", cache: false, service: "JScript", success: gotkey, error: a$.ajaxerror });
            function gotkey(json) {
                if (a$.jsonerror(json)) {
                    ...
                    a$.ajax({
                        type: "GET", async: false, data: { cmd: "login", username: self.newloginusername(), pcypher: cypher, product: product },
                        dataType: "json", cache: false, service: "JScript", error: a$.ajaxerror, success: loaded
                    });
