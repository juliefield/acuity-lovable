...
public partial class _Monitor_base_3C : SitePage
{
    String _connection = "";
    String _connection1 = "";
    String _client = "0";
                ...
                using (DbBaseSql db = new DbBaseSql(_connection, ""))
                {
                    ...
                    */
                    ...
                    if (urlprefix().ToLower() == "km2.")
                    {
                        ...
                        using (DataTable dtkm7 = db1.GetDataTable("select gp.user_id from usr us inner join usr_team ut on ut.user_id=us.user_id inner join team tm on tm.team_id=ut.team_id inner join spvr_grp gp on gp.group_id=tm.group_id where us.user_id='" + db1.reap(lblAgent.Text) + "' and gp.user_id='" + db1.reap(lblViewer.Text) + "'"))
                        {
