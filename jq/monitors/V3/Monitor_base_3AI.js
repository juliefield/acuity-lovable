...
$(document).ready(function () {
    ...
    //Shift AI show/hide into the ng-acuity-ai-page control.
    //The form can then serve as either manual or auto.
    if (($("#lblMode").html() == "new") || (!$("#lblAI_voicefilename").length)) {
        $(".ai-autoqa-show").hide();
        $(".ui-autoqa").hide();
    }
    else {
        $(".ac-show-autoqa").bind("click", function () {
            if ($(this).hasClass("autoqa-active")) {
                $(this).removeClass("autoqa-active");
                $(this).html("Show AutoQA");
                $(".ui-autoqa").hide();
            }
            else {
                $(this).addClass("autoqa-active");
                $(this).html("Hide AutoQA");
                $(".ui-autoqa").show();
            }
        });
    }
    ...
    */
    ...
    $(".ac-submit").bind("click", function () {
        ...
        $(".dv-nonblank").each(function () {
            if ($(this).is(":visible")) {
                if ($(this).attr("disabled") != "disabled") {
                    var minchars = $(this).attr("minchars");
                    if (!a$.exists(minchars)) {
                        if (($(this).val() == "")) {
                        ...
                        var mc = parseInt(minchars, 10);
                        if ($(this).val().length < mc) {
                            alert($(this).attr("qst") + " requires at least " + mc + "characters.");
                            $(this).css("border-width", "2px");
                            $(this).css("border-color", "red");
                            dvpassed = false;
                        }
        ...
        */
        ...
        //TODO: Make this a CSF
        if (a$.urlprefix() == "km2.") {
            if ($("#lblSqfcode").html() == "61") { //Performance Review
                CreateSidekickEntry = true;
            }
            if ($("#lblSqfcode").html() == "80") { //SMART Planner
                CreateSidekickEntry = true;
            }
            if ($("#lblSqfcode").html() == "93") { //Performance Review - SUP PM
                CreateSidekickEntry = true;
            }
        }
        else if (a$.urlprefix() == "walgreens.") {
            if ($("#lblSqfcode").html() == "2") { //Individual Coaching Form
                CreateSidekickEntry = true;
            }
        }
        else if (a$.urlprefix() == "collective-solution.") {
            if ($("#lblSqfcode").html() == "19") { //Performance Review
                CreateSidekickEntry = true;
            }
            if ($("#lblSqfcode").html() == "22") { //SMART Planner
                CreateSidekickEntry = true;
            }
        }
        ...
        var data = {
            ...
            NotifyAgentOnSave: csf.NOTIFY_AGENT_ON_SAVE,
    ...
    $(".ac-delete").bind("click", function () {
        ...
        if ((a$.urlprefix() == "km2.") || (a$.urlprefix() == "walgreens.")) {
    ...
    function client_action_init() {
        ...
        if ((($("#lblRole").html() == "CSR") && (a$.urlprefix() != "performant-healthcare."))
            || ($("#lblAgent").html() == $("#lblViewer").html()))
        //2024-04-11 - DRAFT DEFEAT disabled for performant healthcare:  && (a$.urlprefix() != "performant-healthcare.")
        {
            nodraftview = true;
        ...
        if (($("#lblAgent").html() != $("#lblViewer").html()) || (!csf.RATE_JOURNAL)) {
            $(".StarLink").remove();
        }
        else {
            //TODO: First check to be sure this journal hasn't already been rated (probably something in spinelib)
            $("#coachRating").hide(); //Show this if the journal has already been rated.
            $(".StarLink").show(); //NOTE: If the journal has already been rated, the .StarLink div will have already been removed.


            $(".StarLink").on("click", function () {
                $(".qa-RatingPopup").show();
            });
            $('.qa-close').click(function () {
                alert("Closing without saving.");
                $(".qa-RatingPopup").hide();
            ...
            $('#stars li').on('click', function () {
                ...
                $("#userComments").prop("disabled", false);
                enable_submit();  //NOT DEFINED (How is submit handled for rating popup?)
                ratingValue = parseInt($('#stars li.selected').last().data('value'), 10);
            });
            $('.qa-RatingSubmit').click(function () {
                //alert("Your coaching experience rating has been submitted.");
                var ratingValue = parseInt($('#stars li.selected').last().data('value'), 10);
                var myval = $('#userComments').val();
                myval = myval.replace(/(?:\r\n|\r|\n)/g, '|');
                myval = myval.replace(/\t/g, ' ');
                myval = myval.replace(/\\/g, ' ESCAPE ');
                //TODO: See about eliminating all non-printable characters.
                ...
                //alert("debug: rating = " + ratingValue + ", comments=" + myval);
                var data = {
                    lib: "qa",
                    cmd: "saveRatingByMonitorId",
                    monitorId: $("#lblMonitorId").html(),
                    rating: ratingValue,
                    ratingcomments: myval
                };
                a$.ajax({
                    type: "POST",
                    service: "JScript",
                    ...
                    success: function (json) {
                        ...
                        if (a$.jsonerror(json)) {
                            ...
                            alert("Thank you. Your coaching experience rating has been submitted.");
                        }
                    }
                });
                $(".qa-RatingPopup").hide();
    ...
    function client_load_init(mans) {
        ...
        if (aiprev == 0) { //(a$.gup("aiprev") != "") {
            $(".ac-show-autoqa").addClass("autoqa-active");
            $(".ac-show-autoqa").html("Hide AutoQA");
        }
        else {
            $(".ac-show-autoqa").removeClass("autoqa-active");
            $(".ac-show-autoqa").attr("href", "").html("").attr("disabled", "disabled");
        }
        //$(".ui-autoqa").show();
        ...
        $(".qst").each(function () {
            if (!abort) {
                ...
                if ($(this).parent().prop("tagName").toLowerCase() == "dd") {
                    ...
                    try {
                        if (questiontext == "") {
                            if ($(this).parent().prev().prop("tagName").toLowerCase() == "td") { //1 hop.  TODO: Generalize with TDHOP
    ...
    function load() {
        ...
        if ($("#lblMode").html() == "new") {
            if ((a$.urlprefix() == "km2.") && (csf.KM2_COMPLIANCE_LOCKOUT)) {
        ...
        var data = {
            ...
            formId: formid,
            monitorId: $("#lblMonitorId").html(),
            checkforrating: csf.RATE_JOURNAL && ($("#lblAgent").html() == $("#lblViewer").html()),
            database: "C"
        };
        a$.ajax({
            type: "POST",
        ...
        function monitorloaded(json) {
            ...
            if (a$.jsonerror(json)) {
                ...
                if (json.ratingfound) {
                    //alert("DEVELOPMENT NOTE: A rating has already been submitted for this monitor.\nNormally, the link to rate the message (again) will be removed in this case.\nWhile we're debugging, however, the rating link will still show\nso the rating experience can be tested.");
                    $(".StarLink").remove();
                    $(".rating-is-complete").show();
                }
                    ...
                    for (var i in json.form.mans.answers) {
                        if (json.form.mans.answers[i].friendlyname == qst) {
                            ...
                            if ($(this).attr("type") == "checkbox") {
                                ...
                                var me = this;
                                //if km2 and defectsselected, then append these as options.
                                if (a$.urlprefix() == "km2.") {
                                    if (qst == "DefectsSelected") {
                                        $.each(json.form.mans.answers[i].answertext.split(","), function (i, e) {
                                            $(me).append('<option value="' + e + '">' + e + '</option>');
                                        });
                                    }
                                }
                ...
                if (($("#lblRole").html() == "Admin") && (a$.urlprefix(true).indexOf("mnt") == 0)) {
                    if (qstnf.length) {
    ...
    function init() {
        ...
        if (csf.CDS_ENABLED && ((a$.urlprefix(true).indexOf("mnt") == 0) /* && ($.cookie("TP1Role") == "Admin") */)) {
            ...
            $(".cds-fiddle").show().unbind().bind("click", function () {
                if ($("#lblMode").html() != "new") {
                    ...
                    if ((a$.urlprefix() == "km2.")
                        && ($("#lblViewer").html() != "jeffgack")
                        && (!(($("#lblSqfcode").html() == "7") || ($("#lblSqfcode").html() == "53")))
                    ) {
                        //For KM2, DISABLE COMPLETELY for Monitor_base_3.js while creating Monitor_base_3f.js
                        alert("This is a PRE-HRFLOW monitor, please be sure any edits will be compatible with the HRFLOW common sections.\nAny questions, please contact Jeff Gack");
                    }
        ...
        $(".display-lblAI_entdt").html($("#lblAI_entdt").html());
    ...
    function init_ai() {
        if ($("#lblAI_returntext").html() != "") {
            var json = JSON.parse($("#lblAI_returntext").html());
            var jsontrans;
            if ($("#lblAI_diarization").html() != '') {
                try {
                    jsontrans = JSON.parse($("#lblAI_diarization").html());
                } catch (e) {
                    jsontrans = false;
                }
            }
            ...
            var data = {
                lib: "qa",
                cmd: "getAllMonitorsWithCallId",
                CallId: $("#lblCallid").html() //Note the lower case on id
            };
            a$.ajax({
                type: "POST",
                service: "JScript",
                ...
                success: function (monjson) {
                    var bld = jsonConversion(json, jsontrans, monjson.monitor, aiprev, proposed);
                    $(".ui-autoqa").html(bld);

                    //Rifle through for differences (easier than a double comparison loop).
                    $(".autoqa-column-answer-ai").each(function () {
                        var sameans = $(this).html();
                        var diffound = false;
                        $(".autoqa-column-answer-manual", $(this).parent()).each(function () {
                            if ($(this).html() != sameans) {
                                $(this).addClass("autoqa-answer-different");
                                diffound = true;
                            }
                        });
                        if (diffound) {
                    ...
                    $(".ac-ai-rediarize").on("click", function () {
                        $(this).attr("disabled", "disabled").val("Re-diarizing... The monitor will refresh when complete.");
                        $.post(
                            "https://aitest.acuityapmr.com/request",
                            {
                                prefix: a$.urlprefix().split(".")[0],
                                cmd: "DiarizeFromTranscription",
                                monitorid: $("#lblMonitorId").html()
                            },
                    ...
                    if ($("#lblAI_voicefilename").html() != "") {
                        var sf = "";
                        if (a$.urlprefix() == "da.") {
                            if (($("#lblSqfcode").html() == "4") || ($("#lblSqfcode").html() == "11")) {
                                sf = "/jq/SoundFiles/AUCERA/1295_4/" + $("#lblAI_voicefilename").html();
                            }
                            else if ($("#lblSqfcode").html() == "10") {
                                sf = "/jq/SoundFiles/AUCERA/1340_10/" + $("#lblAI_voicefilename").html();
                            }
                            if (sf != "") {
                                $(".ac-audio").html('<audio controls><source src="' + sf + '" type="audio/mpeg" />Audio Not Supported</audio>');
                            }
                        }
                    }
        ...
        $(".ac-bfa").each(function () {
            ...
            $(" option", this).each(function () {
                if ($(this).val() != "") {
                    ...
                    bld += ' ui-bfa-value-' + $(this).text().replace("/", "");
                    if ($(me).val() == $(this).val()) {
    ...
    function setscore() {
        ...
        if (($("#lblSupervisor").html().toString().toLowerCase() == "ai.system") && ($("#lblAI_ID").html() != "") && ($("#lblAI_PCT_SCORE").html() == null)) {
            $("#lblAI_PCT_SCORE").html("Saving");
            setTimeout(function () {
                var data = {
                    lib: "qa",
                    cmd: "setAIPctScore", //TODO: have this also set the rendered field to 1 in qa_hstry (pass the monitor id) -  This may not be necessary, actually - I can query for pct_scores of null.
                    ai_id: $("#lblAI_ID").html(),
                    monitor_id: $("#lblMonitorId").html(),
                    pct_score: $(".ac-score").html()
                };
                a$.ajax({
                    type: "POST",
                    service: "JScript",
