...
public partial class _Monitor_base_3NG : SitePage
{
    String _connection = "";
    String _connection1 = "";
    String _client = "0";
        ...
        if (!IsPostBack)
        {
            ...
            using (DbBaseSql db1 = new DbBaseSql(_connection1, "")) {
                using (DbBaseSql db = new DbBaseSql(_connection, ""))
                {
                    ...
                    using (DataTable dtan = db1.GetDataTable("select lastnm,firstnm,tm.spvr_user_id,tm.team_id from team tm inner join usr_team ut on ut.team_id=tm.team_id inner join usr on usr.user_id=tm.spvr_user_id where ut.user_id='" + lblAgent.Text + "'"))
                    {
                        if (dtan.Rows.Count >= 1)
                        {
                            ...
                            lblTeam.Text = dtan.Rows[0]["team_id"].ToString();
                        }
                    }
