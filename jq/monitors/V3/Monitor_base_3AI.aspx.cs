...
public partial class _Monitor_base_3AI : SitePage
{
    String _connection = "";
    String _connection1 = "";
    String _client = "0";
    ...
    /*
    void MakeOrAssignControl(String tag, String id, String val)
    {
        HtmlGenericControl mc;
        mc = (HtmlGenericControl)FindControl(id);
        if (mc == null)
        {
    ...
    protected override void OnPreInit(EventArgs e)
    {
        ...
        if (_client == "68") _client = "60"; //KM2TEST Equivalence for content management and monitor behavior.    
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        String aiprev = Context.Request.QueryString["aiprev"];
                ...
                using (DbBaseSql db = new DbBaseSql(_connection, ""))
                {
                    ...
                    using (DataTable dt = db.GetDataTable("select field,val from PRT where guid='" + uid + "'"))
                    {
                        ...
                        using (VirtualStorage cds = new VirtualStorage())
                        {
                            ...
                            cds.Register(ref mp, "PROMPTS." + _client + "." + mysqfcode, false, "", "", "AI_PROMPTS", urlprefix(true).ToLower()); //PROMPTS.26.137
                    ...
                    if (!isnew) //Read in AI_HSTRY variables if available.
                    {
                        //qa_hstry>ai_id->ai_hstry WAS:using (DataTable dai = db1.GetDataTable("select diarization,returnText from goldencrm.dbo.ai_hstry where client='" + lblClient.Text + "' and ai_id in (select ai_id from qa_hstry where monitor_id='" + lblMonitorId.Text + "')"))
                        //monitor_id->ai_hstry IS:

                        String mysql = "select ai_id,convert(varchar,entdt,22) as myentdt,case when pct_score is null then -1000.0000 else pct_score end as PCT_SCORE,Transcription,TranscriptionDev,Diarization,prePrompt,monitorDefPrompt,postPrompt,scriptGuidance,VoiceFileName,returnText from goldencrm.dbo.ai_hstry where client='" + lblClient.Text + "' and monitor_id='" + lblMonitorId.Text + "' and querytype in ('MONITOR_FROM_TRANSCRIPT','MONITOR_REDO_FROM_PROMPTS') order by entdt desc";
                        //String mysql = "select ai_id,convert(varchar,entdt,22) as myentdt,case when pct_score is null then -1000.0000 else pct_score end as pct_score,case when transcriptiondev<>'' then transcriptiondev else transcription end as transcription,case when diarization<>'' then diarization else case when transcriptiondev<>'' then transcriptiondev else transcription end end as diarization,prePrompt,monitorDefPrompt,postPrompt,scriptGuidance,voicefilename,returnText from goldencrm.dbo.ai_hstry where client='" + lblClient.Text + "' and monitor_id='" + lblMonitorId.Text + "' and querytype in ('MONITOR_FROM_TRANSCRIPT','MONITOR_REDO_FROM_PROMPTS') order by entdt desc";
                        //String mysql = "select top 1 ai_id,diarization,voicefilename,returnText from goldencrm.dbo.ai_hstry where client='" + lblClient.Text + "' and monitor_id='" + lblMonitorId.Text + "' and querytype in ('MONITOR_FROM_TRANSCRIPT') order by entdt desc";
                        using (DataTable dai = db1.GetDataTable(mysql))
                        {
                            HtmlGenericControl mc;
                            int aimatch = 0;
                            if (aiprev != null)
                            {
                                aimatch = Convert.ToInt32(aiprev);
                            }

                            MakeOrAssignControl(bodyDiv,"label","lblAI_totalCount", dai.Rows.Count.ToString());

                            int aiidx = 0;
                            String aiid = "";
                            foreach (DataRow row in dai.Rows)
                            {
                                if (aiidx == aimatch)
                                {
                                    //lblAI_ID.Text = row["ai_id"].ToString();
                                    aiid = row["ai_id"].ToString();
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_ID", row["ai_id"].ToString());
                                    if (row["pct_score"].ToString() != "-1000.0000")
                                    {
                                        //lblAI_PCT_SCORE.Text = row["PCT_SCORE"].ToString();
                                        MakeOrAssignControl(bodyDiv, "label", "lblAI_PCT_SCORE", row["PCT_SCORE"].ToString());
                                    }
                                    /*
                                    lblAI_Diarization.Text = row["diarization"].ToString();
                                    lblAI_Transcription.Text = row["transcription"].ToString();
                                    lblAI_returnText.Text = row["returnText"].ToString();
                                    lblAI_VoiceFileName.Text = row["voicefilename"].ToString();
                                    lblAI_entdt.Text = row["entdt"].ToString();
                                    */
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_diarization", row["Diarization"].ToString());
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_transcription", row["Transcription"].ToString());
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_transcriptiondev", row["TranscriptionDev"].ToString());
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_returntext", row["returnText"].ToString());
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_voicefilename", row["voicefilename"].ToString());
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_entdt", row["myentdt"].ToString());

                                    //TODO: The prompt fields need to be dynamically built based on records in AI_PROMPTS_SEQUENCE joined with AI_PROMPTS_STRINGTABLE
                                    //TODO: Add sequence_id field to AI_HSTRY.
                                    // select apst.val from AI_PROMPTS_SEQUENCE aps inner join AI_PROMPTS_STRINGTABLE apst on apst.id=aps.idprompt 
                                   
                                    /*
                                    lblAI_prePrompt.Text = row["prePrompt"].ToString();
                                    lblAI_monitorDefPrompt.Text = row["monitorDefPrompt"].ToString();
                                    lblAI_postPrompt.Text = row["postPrompt"].ToString();
                                    lblAI_scriptGuidance.Text = row["scriptGuidance"].ToString();
                                     * */

                                    //These will be overwritten below, still assigning during the transition.
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_prePrompt", row["prePrompt"].ToString());
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_monitorDefPrompt", row["monitorDefPrompt"].ToString());
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_postPrompt", row["postPrompt"].ToString());
                                    MakeOrAssignControl(bodyDiv, "label", "lblAI_scriptGuidance", row["scriptGuidance"].ToString());

                                    mysql = "select ap.prompt_name as promptName,apst.promptText as promptText from goldencrm.dbo.ai_step_log asl inner join goldencrm.dbo.ai_process_step aps on aps.id=asl.idStep inner join goldencrm.dbo.ai_prompts ap on ap.idprompts = aps.prompts_id inner join goldencrm.dbo.ai_prompt_stringtable apst on apst.id=ap.prompt_string_id where asl.ai_id=" + aiid + " and ap.prompt_string_id<>0 order by ap.seq";
                                    using (DataTable dap = db1.GetDataTable(mysql))
                                    {
                                        foreach (DataRow rowP in dap.Rows)
                                        {
                                            /*
                                            mc = (HtmlGenericControl)FindControl("lblAI_" + rowP["promptName"].ToString());
                                            mc.InnerHtml = rowP["promptText"].ToString();
                                            */
                                            MakeOrAssignControl(bodyDiv, "label", "lblAI_" + rowP["promptName"].ToString(), rowP["promptText"].ToString());
                                        }
                                    }

                                    /* TODO: Generalize this from above
                                    lblAI_prePrompt.Text = row["prePrompt"].ToString();
                                    lblAI_monitorDefPrompt.Text = row["monitorDefPrompt"].ToString();
                                    lblAI_postPrompt.Text = row["postPrompt"].ToString();
                                    lblAI_scriptGuidance.Text = row["scriptGuidance"].ToString();
