...
$(document).ready(function () {
    ...
    window.parent['QASaveConfirmed_' + formid + "_" + examinee] = "";

    //alert("debug: Will be polling for: QASubmit_" + formid + "_" + examinee);
    ...
    function checksubmit() {
        if (!submitted) {
            if (window.parent['QASubmit_' + formid + "_" + examinee] != "") {
                submitted = true;
                //alert("debug: pre-Submitting! - masterid = " + masterid);
    ...
    var csf = {
        FORM_SIZE: { height: "471px", width: "651px" },
        JOURNAL_SIZE: { height: "100%", width: "700px" },
    ...
    try {
        window.parent.document.getElementById('SidekickSubformID').width = csf.FORM_SIZE.width;
        window.parent.document.getElementById('SidekickSubformID').height = csf.FORM_SIZE.height;
    } catch(e) {};
    try {
        window.parent.document.getElementById('CustomFormPlaceholderID').style.width = csf.FORM_SIZE.width;
        window.parent.document.getElementById('CustomFormPlaceholderID').style.height = csf.FORM_SIZE.height;
    } catch(e) {};
    try {
        window.parent.document.getElementById('JournalPopupID').style.width = csf.JOURNAL_SIZE.width;
    } catch(e) {};
    ...
    function harvestQuestions() {
        ...
        //TODO: look for qsttable and harvest it.
    ...
    function processSubmit() {
        ...
        if (alreadylive == "No") {
            var userlist = "";
            $("#huddleuserlistchart .li-checkbox input:checked").each(function() {
                userlist += ((userlist.length) ? "," : "") + $(this).attr("userid");
            });
            $(".ac-user-list").val(userlist);
        }

        var mans = harvestQuestions();

        if (formid == 243) {
            //First Test for qsttable concept.  The tilda separation for columns greatly reduces the complexity (multiple is designed for a linked list).
            /*
            mans.multiple.push(
                { answers: [{ friendlyname: "gridval", answertext: "cell 0 0~cell 0 1", score:0, value: "", version: 0 }], multiple: [] }
            );
            mans.multiple.push(
                { answers: [{ friendlyname: "gridval", answertext: "cell 1 0~cell 1 1", score:0, value: "", version: 0 }], multiple: [] }
            );
            */

                    $(".qsttable").each(function() {
                       var me = this;
                       var fieldname = $(this).attr("qsttable");
                       var ri=0;
                       var ci = 0;
                       var cs = "";
                       $("thead tr", me).each(function() {
                         ci = 0;
                         cs = "";
                         $("th", this).each(function() {
                           $("span",this).eq(0).each(function() {                                
                               //replaceme: $(this).html("H:r1=" + $(this).attr("row") + ",r2=" + $(this).attr("col") );
                               //$(this).html(manstext(fieldname,parseInt($(this).attr("row"),10),parseInt($(this).attr("col"),10)) );
                               if (cs != "") cs += "~";
                               cs += $(this).html();
                               //$(this).html('<span style="color:green" qsttable="' + scope.qsttable + '" row="' + ri + '" col="' + ci + '">' + $(this).html() + '</span>');
                           });
                           ci += 1;
                         });
                         mans.multiple.push({ answers: [{ friendlyname: "gridval", answertext: cs, score:0, value: "", version: 0 }], multiple: [] } );
                         ri +=1;                                        
                       });
                       $("tbody tr", me).each(function() {
                         ...
                         $("td", this).each(function() {
                            var spanfound = false;
                            $("span",this).eq(0).each(function() {                                
                                //replaceme: $(this).html("C:r1=" + $(this).attr("row") + ",r2=" + $(this).attr("col") );
                                //$(this).html(manstext(fieldname,parseInt($(this).attr("row"),10),parseInt($(this).attr("col"),10)) );
                                //$(this).html('<span style="color:green" qsttable="' + scope.qsttable + '" row="' + ri + '" col="' + ci + '">' + $(this).html() + '</span>');
                                if (cs != "") cs += "~";
                                cs += $(this).html();
                            ...
                            if (!spanfound) {
                                $("input",this).eq(0).each(function() {                                
                                    //replaceme: $(this).val("I:r1=" + $(this).attr("row") + ",r2=" + $(this).attr("col") );
                                    //$(this).val(manstext(fieldname,parseInt($(this).attr("row"),10),parseInt($(this).attr("col"),10)));
                                    if (cs != "") cs += "~";
                                    cs += $(this).val();
        ...
        //alert("debug:saveQaForm, formid=" + formid + ",masterid=" + masterid);
        ...
        var data = {
            ...
            database: "C",
            examinee: $("#lblAgent").html(),
            score: "0", //(($(".ac-score").html() != null) ? $(".ac-score").html().replace("%", "") : "0"),
            value: "", //(($(".ac-autofail").html() != null) ? (($(".ac-autofail").eq(0).css("display") != "none") ? "Auto Fail" : "") : ""),
            //answers: answers,
            mans: mans,
            acknowledgement: csf.AGENT_ACKNOWLEDGEMENTS_ENABLED//Left machinery in though.
        };
        if ($("#lblMode").html() != "new") {
        ...
        function monitorsaved(json) {
            if (a$.jsonerror(json)) {
                ...
                if (!acknowledgingnow) {
                    ...
                    //2024-04-20 - Do Multi Save
                    if (alreadylive == "No") {
                        var livenow = myval = $('input[name="liveorno"]:checked').val();
                        if (livenow == "Yes") {
                            //alert("debug: Doing Multisave");
                            var userlist = "";
                            $("#huddleuserlistchart .li-checkbox input:checked").each(function() {
                                userlist += ((userlist.length) ? "," : "") + $(this).attr("userid");
                            });
                            //alert("debug: saving attempt for: " + userlist);
                            var data = {
                                lib: "spine",
                                cmd: "makeChildJournalRecords",
                                masterId: masterid,
                                userlist: userlist,
                                category: "Huddle Record" //TODO: Generalize this
                            };
                            a$.ajax({
                                type: "POST",
                                service: "JScript",
                                ...
                                success: function(json) {
                                    if (a$.jsonerror(json)) {
                                        ...
                                        //alert("debug: Child records saved (?)");
                                    }
    ...
    function client_load_init(mans) {
        ...
        alreadylive = $('input[name="liveorno"]:checked').val();

        if (alreadylive != "No") {
            $(".save-section").hide();
        }

        //alert("debug: alreadylive=" + alreadylive);
        if ($("#lblMode").html() != "new") {
            //TODO: Set the values in huddleuserlistchart based on ac-user-list (uncheck the boxes that aren't in this list)
            var ul = $(".ac-user-list").val();
            var uls = ul.split(",");
            setTimeout(function() { //TODO: Trigger this based on completion of huddleuserlist cid.
                $("#huddleuserlistchart .li-checkbox input").each(function() {
                    var found = false;
                    for (var u in uls) {
                        if (uls[u] == $(this).attr("userid")) {
                            found = true;
                        }
                    }
                    if (!found) {
            ...
            if (alreadylive == "Yes") { //Disable the checkboxes
                setTimeout(function() { //TODO: Trigger this based on completion of huddleuserlist cid.
                    $("#huddleuserlistchart .li-checkbox input").each(function() {
    ...
    function load(publishcompletion) {
        ...
        if ($("#lblMode").html() == "new") {
            ...
            ko.postbox.subscribe("ngAcuityReportComplete", function(newValue) {
              $(".report-loading").hide(); //cheating
            });
            return;
        }
        var data = {
        ...
        function monitorloaded(json) {
            ...
            if (a$.jsonerror(json)) {
                ...
                //QA Entry Date (added 2022-04-18)
                if (json.form.entrydate) {
                    $("#lblQAEntryDate").html(json.form.entrydate);
                    $(".display-lblQAEntryDate").html($("#lblQAEntryDate").html());
                }
                    ...
                    for (var i in json.form.mans.answers) {
                        if (json.form.mans.answers[i].friendlyname == qst) {
                            ...
                            if ($(this).attr("type") == "checkbox") {
                                ...
                                var me = this;
                                $.each(json.form.mans.answers[i].answertext.toString().split(","), function (i, e) {
                                    $("option[value='" + e + "']", me).prop("selected", true);
                                });
                            }
                            else if ($(this).prop("tagName") == "PRE") {
                                $(this).html(json.form.mans.answers[i].answertext);
                            }
                            else {
                ...
                function manstext(qsttable,row,col) {
                    var bld = "N/A";
                    if (row < json.form.mans.multiple.length) {
                        var ans = json.form.mans.multiple[row].answers;
                        for (var a in ans) {
                            if (ans[a].friendlyname == qsttable) {
                                csp = ans[a].answertext.split("~");
                                if (col < csp.length) {
                                    bld = csp[col];
                                    break;
                                }
                ...
                ko.postbox.subscribe("ngAcuityReportComplete", function(newValue) {
                    ...
                    repeat += 1;
                    //alert("debug: report completed: " + newValue);
                    $(".qsttable").each(function() {
                       var me = this;
                       var fieldname = $(this).attr("qsttable");
                       var ri=0;
    ...
    function init() {
        ...
        if (a$.gup("editmode") == "true") {
            ...
            mgrs = $("#lblAgentTeamMembers").html();
            //alert("debug:AgentTeamMembers=" + mgrs);
            mgrss = mgrs.toString().split("|");
            for (var m in mgrss) {
                var ms = mgrss[m].toString().split("~");
                $(".combo-agentteammembers").append('<option ' + ((ms[0] == groupleader) ? "selected" : "") + ' value="' + ms[0] + '">' + ms[1] + "</option>");
            }
    ...
    function init_control() {
        ...
        $(".ac-setup-form").unbind().bind("click", function () {
            ...
            //alert("debug: Saving, formid = " + formid);
            ...
            function formhasbeensetup(json) {
                if (a$.jsonerror(json)) {
                    ...
                    if (!a$.exists(json.formId)) {
                        ...
                        //location.reload();
                        $(".ac-submit").trigger("click"); //Saves everything in place.
                        alert("Setup Successful, please close the form and select again.");
                    }
                }
            }
            return false;
    ...
    function return_to_v1() {
        window.parent['QASaveConfirmed_' + formid + "_" + examinee] = "YES";
        window.close();
    }
