$(document).ready(function () {
    ...
    function loadtags() {
        var curtag = $(".mon-field-trendcategory").val().replace(/\& /g, "&amp; ");
        ...
        $(".combo-tags").empty();
        $(".combo-tags").append('<option value=""></option>');
        for (var t in tagss) {
            var sel = "";
            if (tagss[t] == curtag) sel = ' selected="selected" ';
            $(".combo-tags").append('<option' + sel + ' value="' + tagss[t] + '">' + tagss[t] + "</option>");
        }
        $(".new-tag").unbind().bind("click", function () {
            $(".mon-field-trendcategory").val("").show();
            $(".select-tag").show();
            $(".combo-tags").hide();
            $(".new-tag").hide();
        });
        $(".select-tag").unbind().bind("click", function () {
            $(".mon-field-trendcategory").val($(".combo-tags").val()).hide();
            $(".select-tag").hide();
            $(".combo-tags").show();
            $(".new-tag").show();
        });
        ...
        $(".combo-tags").bind("change", function () {
            $(".mon-field-trendcategory").val($(this).val());
        });
    }
    ...
    function changeletteridlist(manuallychanging) {
        ...
        if (manuallychanging) {
            if ($("#lblMode").html() == "new") {
                if (false) { //(lt == "GHP") { //2022-12-21 - ALL MONITORS TO BE RELEASED IMMEDIATELY.
            ...
            function monitorloaded(json) {
                ...
                if (a$.jsonerror(json)) {
                    ...
                    if (qanotes.toLowerCase().indexOf("hold") >= 0) {
                        /* //2022-12-21 - All release dates are immediate now, so a hold-released monitor will be released.
