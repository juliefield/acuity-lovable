...
public partial class _MonitorLink_base_1POP : SitePage
{
    String _connection = "";
    String _connection1 = "";
    String _client = "0";
    ...
    protected override void OnPreInit(EventArgs e)
    {
        ...
        {
            using (DataTable ddt = db1.GetDataTable("select dbo.client() as client"))
            {
        ...
        if (!IsPostBack) {
            ...
            using (DbBaseSql db1 = new DbBaseSql(_connection1, "")) {
                using (DbBaseSql db = new DbBaseSql(_connection, ""))
                {
                    ...
                    {
                        lblMode.Text = "update";
                        //Added 2021-09-21: The lblFormId needs to be retrieved via the master ID (from qa_hstry) for monitor links (since it could be zero to account for global CDS).
                        //The formid used for CDS is as passed (0 possibly).
                        //lblFormId.Text = "";
                        using (DataTable dt = db1.GetDataTable("select idform from qa_hstry where master_id='" + db1.reap(masterid) + "'"))
                        {
                            foreach (DataRow row in dt.Rows)
                            {
                                lblFormId.Text = row["idform"].ToString();
                                break;
                            }
                        }
                        if (lblFormId.Text == "")
                        {
                            throw new Exception("Master ID Not Found");
                        }
                    }
                    ...
                    */
                     ...
                     * */
                        ...
                        using (VirtualStorage cds = new VirtualStorage())
                        {
                            ...
                            lblBodyId.Text = "MONLINK." + _client + "." + formid;
