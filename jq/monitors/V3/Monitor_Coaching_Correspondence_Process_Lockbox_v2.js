$(document).ready(function () {
    ...
    $(".mon-submit").click(function () {
        if (!$(".mon-sys-hold").is(":checked")) {
            var d = new Date();
            if ($("#inputEnteredDate").val() == "") {
                $("#inputEnteredDate").val((d.toLocaleDateString() + " " + d.toLocaleTimeString()).replace(/\?/g, ""));
            }
        } else {
            $("#inputEnteredDate").val("");
        }
    });
    ...
    var passed_monitortype = a$.gup("monitortype");
    var passed_original_username = a$.gup("original_username");
    var passed_original_userid = a$.gup("original_userid");
    var passed_original_date = a$.gup("original_date");
    ...
    $(".show-score, .show-disp").show();
    $(".cb-cplb").bind("change", function () {
        if ($(this).prop("checked")) {
            $(".combo-lettertypes").val("N/A-CPLB");
            $(".tbl-cplb, .show-scorecplb, .show-dispcplb").show();
            $(".show-score, .show-disp, .tbl-Rationale, .tbl-CaseLevel, .tbl-CRCResponse, .tbl-PHI").hide();
        }
        else {
            $(".show-score, .show-disp, .tbl-Rationale, .tbl-CaseLevel, .tbl-CRCResponse, .tbl-PHI").show();
    ...
    $("#coachingCheckBox").bind("change", function () {
        if (($(this).prop("checked")) && ($("#lblMode").html() != "new")) {
            $(".show-coaching").show();
        } else {
            $(".show-coaching").hide();
        }
    ...
    function handleRebuttalChange(fieldSelector, timestampFieldSelector, level) {
        var field = document.querySelector(fieldSelector);
        var timestampField = document.querySelector(timestampFieldSelector);
        function updateTimestamp() {
            const value = field.value;
            if (value) {
                var d = new Date();
                timestampField.value = (d.toLocaleDateString() + " " + d.toLocaleTimeString()).replace(/\?/g, "");
            } else {
                alert(`Please enter rebuttal level ${level} comments.`);
                timestampField.value = "";
            }
        }
        field.addEventListener('input', updateTimestamp);
        field.addEventListener('paste', function() {
            setTimeout(updateTimestamp, 0);
        });
    }    
    handleRebuttalChange(".mon-field-rebuttal", ".qst-rebuttal-timestamp", 1);
    handleRebuttalChange(".mon-field-rebuttalresponse", ".qst-rebuttal-response-timestamp", 1);
    handleRebuttalChange(".mon-field-rebuttal-b", ".qst-rebuttal-timestamp-b", 2);
    handleRebuttalChange(".mon-field-rebuttalresponse-b", ".qst-rebuttal-response-timestamp-b", 2);
    handleRebuttalChange(".mon-field-rebuttal-c", ".qst-rebuttal-timestamp-c", 3);
    handleRebuttalChange(".mon-field-rebuttalresponse-c", ".qst-rebuttal-response-timestamp-c", 3);
    ...
    $(".cheapscore").each(function () {
        if ($(this).prop("value") == "Fail") {
            ...
            $(this).closest('tr').find('textarea.qst-comments').addClass('dv-nonblank');

        }
        else {
            $(this).parent().parent().next().children().hide();
            $(this).closest('tr').find('textarea.qst-comments').removeClass('dv-nonblank');
        }
    });
    ...
    // $(".cheapscore").on("change", function () {
    //     if ($(this).val() == "Fail") {
    //         $(this).parent().parent().next().children().show().prop('required', true);
    //     } else {
    //         $(this).parent().parent().next().children().hide().prop('required', false);
    //     }
    // });    
    ...
    //     if (score < 0) score = 0;
    //     if (disp != "Auto Fail") {
    //         disp = "" + (((score / 448) * 100.0).toFixed(2) + "%");
    //     }
    //     $(".show-score").html(score + "/" + "448");
    //     $(".ac-score").html(score + "/" + "448");
    ...
    $(".mon-answer select").bind("change", function () {
        ...
        if (($("#lblRole").html() != "Quality Assurance") && (score < 100) && ($("#lblMode").html() != "new")) {
            $("#coachingCheckBox").prop("checked", true).trigger("change");
        } else {
            $("#coachingCheckBox").prop("checked", false).trigger("change");
        }
    ...
    $(".mon-answer-cplb select").bind("change", function () {
        ...
        $(".show-scorecplb").html(scorecplb + "/" + "100");
        // if (($("#lblRole").html() != "Quality Assurance")) {
        //     if (scorecplb < 100) {
        //         $("#coachingCheckBox").prop("checked", true).trigger("change");
        //     }
    ...
    $(".tbl-Rationale select").bind("change", function () {
        var numRationale = 0;
        var denRationale = 368;
        var RationaleScore = 100;
        var RationaleFindings = 0;
        $(".tbl-Rationale select").each(function () {
            if ($(this).val() == "Fail") {
                RationaleScore -= parseInt($(this).attr("score"), 10);
                $(this).next(".qst-comments").prop("required", true);
                RationaleFindings += 1;
            }
            if ($(this).val() == "Pass") {
                numRationale += parseInt($(this).attr("score"), 10);
                $(this).next(".qst-comments").prop("required", false);
            }
            $("#RationaleFindings").html("Findings=" + RationaleFindings);
        });
        if (RationaleScore < 0) RationaleScore = 0;
        // RationaleScore = (((numRationale / denRationale) * 100).toFixed(0) + "%");
        // $("#numRationale").html(numRationale);
        // $("#denRationale").html(denRationale);
        $("#RationaleScore").html(RationaleScore);
        // if (($("#lblRole").html() != "Quality Assurance")) {
        //     if (RationaleScore < 100) {
        //         $("#coachingCheckBox").prop("checked", true).trigger("change");
        //     }
        //     else {
        //         $("#coachingCheckBox").prop("checked", false).trigger("change");
    ...
    $(".tbl-CaseLevel select").bind("change", function () {
        var numCaseLevel = 0;
        var denCaseLevel = 114;
        var CaseLevelScore = 100;
        var CaseLevelFindings = 0;
        $(".tbl-CaseLevel select").each(function () {
            if ($(this).val() == "Fail") {
                CaseLevelScore -= parseInt($(this).attr("score"), 10);
                $(this).closest(".qst-comments").prop("required", true);
                CaseLevelFindings += 1;
            }
            if ($(this).val() == "Pass") {
                numCaseLevel += parseInt($(this).attr("score"), 10);
                $(this).closest(".qst-comments").prop("required", false);
            }
            $("#CaseLevelFindings").html("Findings=" + CaseLevelFindings);
        });
        if (CaseLevelScore < 0) CaseLevelScore = 0;
        // CaseLevelScore = (((numCaseLevel / denCaseLevel) * 100).toFixed(0) + "%");
        // $("#numCaseLevel").html(numCaseLevel);
        // $("#denCaseLevel").html(denCaseLevel);
        $("#CaseLevelScore").html(CaseLevelScore);
        // if (($("#lblRole").html() != "Quality Assurance")) {
        //     if (CaseLevelScore < 100) {
        //         $("#coachingCheckBox").prop("checked", true).trigger("change");
        //     }
        //     else {
        //         $("#coachingCheckBox").prop("checked", false).trigger("change");
    ...
    function buttonsforanswers() {
        ...
        $(".mon-answer select").each(function () {
            if (!$(this).parent().hasClass("mon-answer-special")) {
                ...
                if (($(this).attr("score") == "6") || ($(this).attr("score") == "12") || ($(this).attr("score") == "16")) {
    ...
    $(".mon-sys-rebuttalresponsesuccess").bind("change", function () {
        rebuttalresponsesuccesschanged = true;
        checksubmit();
    });

    $(".mon-sys-rebuttalresponsesuccess-b").bind("change", function () {
        rebuttalresponsesuccesschangedb = true;
        checksubmit();
    });

    $(".mon-sys-rebuttalresponsesuccess-c").bind("change", function () {
    ...
    if ($("#lblMode").html() == "new") {
        ...
        $('.mon-sys-rebuttalresponsesuccess option[value="Yes"]').remove();
        $('.mon-sys-rebuttalresponsesuccess-b option[value="Yes"]').remove();
        $('.mon-sys-rebuttalresponsesuccess-c option[value="Yes"]').remove();
    }
    /*
    if ($("#lblMode").html() != "new") {
        const selectors = [
            '.mon-sys-rebuttalresponsesuccess',
            '.mon-sys-rebuttalresponsesuccess-b',
            '.mon-sys-rebuttalresponsesuccess-c'
        ];
        selectors.forEach(selector => {
            if ($(selector).val() != "Yes") {
                selectors.forEach(sel => {
                    $(`${sel} option[value="Yes"]`).remove();
                });
            }
        });
    }
    */
    if ($("#lblMode").html() == "new") {
        if (passed_monitortype != "") {
            //alert("debug: assigning monitortype: " + passed_monitortype);
            $(".qst-monitortype").val(passed_monitortype);
            $(".qst-original-username").val(passed_original_username);
            $(".qst-original-userid").val(passed_original_userid);
            $(".qst-original-date").val(passed_original_date);
        }
    }
    ...
    if ($("#lblMode").html() == "new") { //Don't reset if this is an already-saved monitor.
        ...
        $("#setHold1, #setHold2, #setHold3").change(function () {
            if (($("#setHold1").val() == "Fail") || ($("#setHold2").val() == "Fail") || ($("#setHold3").val() == "Fail")) {
                $(".mon-sys-hold").prop("checked", true);
                alert("On HOLD due to Other=Fail");
                $(".mon-field-reasonnotes").prop("disabled", false).addClass("dv-nonblank");
                $(".mon-field-releasedate").val("");
            } else {
                $(".mon-sys-hold").prop("checked", false);
                $(".mon-field-reasonnotes").prop("disabled", true).removeClass("dv-nonblank");
                // $(".mon-field-releasedate").val($("#lblToday").val());
                $("#lblCalldate").val($("#lblToday").val());
                $(".mon-field-releasedate").prop("disabled", true);
    ...
    $(".mon-submit,.mon-rebuttal,.mon-rebuttalresponse,.mon-coaching,.mon-rebuttal-b,.mon-rebuttalresponse-b,.mon-rebuttal-c,.mon-rebuttalresponse-c").bind("click", function () {
        ...
        $(".qst").each(function () {
            ...
            if ($(this).attr("qst") == "RebuttalResponseSuccess") {
                if ($(this).val() != "") {
                    myval = ($(".mon-sys-rebuttalresponsesuccess").find(":selected").val());
                }
                else {
                    myval = "";
                }
            ...
            if ($(this).attr("qst") == "RebuttalResponseSuccess-b") {
                if ($(this).val() != "") {
                    myval = ($(".mon-sys-rebuttalresponsesuccess-b").find(":selected").val());
                }
                else {
                    myval = "";
                }
            ...
            if ($(this).attr("qst") == "RebuttalResponseSuccess-c") {
                if ($(this).val() != "") {
                    myval = ($(".mon-sys-rebuttalresponsesuccess-c").find(":selected").val());
                }
                else {
                    myval = "";
                }
            ...
            if ($(this).attr("qst") == "holdBtn") {
                if ($(this).is(":checked")) {
                    ...
                    myval = "";
                    if ($(".monitor-table select").val() == "Pass") {
        ...
        var data = {
            ...
            cmd: "saveQaForm",
            formId: 373,
            sqfCode: 67, //66, //Developed as 66, changed to 61.
            kpi: mykpi,
            kpiSet: [84],
            database: "C",
            examinee: $("#lblAgent").html(),
            score: $(".mon-sys-score" + ($(".cb-cplb").eq(0).prop("checked") ? "-cplb" : "")).html().replace("%", "").split("/")[0],
                        ...
                        for (var i in json.form.answers) {
                            if ((json.form.answers[i].friendlyname == qst) || (json.form.answers[i].idquestion == qst)) { //Performant exception, qst must match on either idquestion or friendlyname
                                if (qst == "LetterID") {
                                    ...
                                    if (json.form.answers[i].answertext == ($(".mon-sys-rebuttalresponsesuccess option:selected").val())) {                                        
                                    ...
                                    if (json.form.answers[i].answertext == ($(".mon-sys-rebuttalresponsesuccess-b option:selected").val())) {
                                    ...
                                    if (json.form.answers[i].answertext == ($(".mon-sys-rebuttalresponsesuccess-c option:selected").val())) {
                                    ...
                                    d2.setTime(d2 - (((24 - 9) + 24 + 24 /* Added 2025-01-27: + 24 End Add */ + weekendhours) * 60 * 60 * 1000)); //Midnight on Wednesday following the release date (backing up current date to compare).
                    ...
                    */
                    ...
                    if ($("#lblCallid").html().indexOf("-QA") !== -1) {
                        ...
                        if ($("#rebuttalresponseme").is(":visible")) {
                            ...
                            $(".mon-sys-rebuttalresponsesuccess").attr("disabled", "disabled");                           
                        }
                        if ($(".mon-field-rebuttalresponse").val() != "") {
                            $(".mon-field-rebuttalresponse").attr("disabled", "disabled");
                            $("#rebuttalresponseme").hide().attr("hidden", "true");
                            $(".mon-sys-rebuttalresponsesuccess").attr("disabled", "disabled");
                        ...
                        if ($("#rebuttalmeb").is(":visible")) {
                            if ($("#lblViewer").html() == $("#lblAgent").html()) {
                                $(".mon-sys-rebuttalresponsesuccess-b").attr("disabled", "disabled");
                        ...
                        if ($(".mon-field-rebuttal-b").val() == "") {  
                            ...
                            $(".mon-field-rebuttal-b").attr("disabled", "disabled");
                            $("#rebuttalmeb").hide().attr("hidden", "true");                            
                        ...
                        if ($(".mon-field-rebuttalresponse-b").val() != "") {
                            $(".mon-field-rebuttalresponse-b").attr("disabled", "disabled");
                            $("#rebuttalresponsemeb").hide().attr("hidden", "true");
                            $(".mon-sys-rebuttalresponsesuccess-b").attr("disabled", "disabled");
                        ...
                        if ($("#rebuttalresponsemec").is(":visible")) {
                            ...
                            // $(".mon-field-rebuttalresponse-c").attr("disabled", "disabled"); // -->2/3/2025 B.G. enable rebuttal response level 3 comments
                        }
                        if ($(".mon-field-rebuttalresponse-c").val() != "") {
    ...
    //2024-08-13 - Show delete for specific users.
    $("#deleteme").hide();
    var userid=$("#lblViewer").html().toString().toLowerCase();
    switch (userid) {
        case "jjohnson2":
        case "nreid":
        case "phoard":
...
"WN - Wrong Number"
    ...
    //2024-04-10: ATTEMPT TO GENERALIZE THE FORM SETUP WITHOUT USING THE MONITOR BASE
    ...
    function harvestQuestions() {
        ...
        $(".qst").each(function () {
            ...
            if (typeof myval === "string") {
                ...
                // Replace backslashes with 'ESCAPE'
                myval = myval.replace(/\\/g, ' ESCAPE ');
                // Remove all non-printable characters
                myval = myval.replace(/[^\x20-\x7E]/g, "*");
            }
    ...
    function setupform_hack() { //$(".ac-setup-form").unbind().bind("click", function () {
        ...
        var data = {
            lib: "qa",
            cmd: "setupQaForm",
            formId: 373,  //If "0", then create a new form, otherwise just add new questions.
            mans: mans,
            database: "C",
            ...
            sqfCode: 67, //$("#lblSqfcode").html(),
            sqfName: "Performant 67" //$("#lblSqfname").html()
        };
        a$.ajax({
            type: "POST",
            service: "JScript",
        ...
        function formhasbeensetup(json) {
            if (a$.jsonerror(json)) {
                ...
                if (!a$.exists(json.formId)) {
                    ...
                    alert("debug: Form Setup was successful");
    ...
    // if (confirm("debug: Invoke SetupForm? (comment this line out when done)")) setupform_hack();  
    // <----Uncomment to update idquestions
