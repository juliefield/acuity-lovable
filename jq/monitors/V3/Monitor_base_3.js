...
$(document).ready(function () {
    ...
    */
    ...
    function load() {
        ...
        /*
        //Read in qstcats (Special category in ClientQuestionCategoryName field in qst)
        var foundcat = false;
        if (mans) {
        $(".qst").each(function () {
        var qst = $(this).attr("qst");
        for (var i in mans.answers) {
        if (mans.answers[i].friendlyname == qst) {
        if (a$.exists(mans.answers[i].ClientQuestionCategoryName)) {
        if (mans.answers[i].ClientQuestionCategoryName != "") {
        $(this).attr("qstcat", mans.answers[i].ClientQuestionCategoryName);
        }
        }
        }
        }
    ...
    function init() {
        ...
        if ((a$.urlprefix() == "km2.") && (csf.KM2_COMPLIANCE_LOCKOUT) && ($("#lblComplianceAllowed").html() != "Yes")) {
            $(".display-lblSupervisorName").hide();
            $(".display-lblSupervisorName").closest("td").prev().hide();
        }
        else {
        ...
        $(".display-lblActionRequired").html($("#lblActionRequired").html());
        $(".display-lblActionRequiredSupervisorName").html($("#lblActionRequiredSupervisorName").html());
