...
public partial class _Subform_base_1_child : SitePage
{
    String _connection = "";
    String _connection1 = "";
    String _client = "0";
    ...
    protected override void OnPreInit(EventArgs e)
    {
        ...
        if (formid != "")
        {
            ...
            {
                using (DataTable ddt = db.GetDataTable("select client from qafrm where id=" + db.reap(formid)))
                {
        ...
        {
            // _client = CONFMGR.AppSettings(urlprefix() + "client"); //Default to global (0)
        }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        // form dataview masterid examinee examiner prefix
        String formid = Context.Request.QueryString["formid"];
        String masterid = Context.Request.QueryString["masterid"];
        String examinee = Context.Request.QueryString["examinee"];
        String examiner = Context.Request.QueryString["examiner"];
        String parentmasterid = Context.Request.QueryString["parentmasterid"];
            
        if (!IsPostBack) {

            lblClient.Text = _client;
            lblMasterId.Text = masterid;
            lblAgent.Text = examinee;
            lblExaminer.Text = examiner;
            lblParentMasterId.Text = parentmasterid;
            ...
            using (DbBaseSql db1 = new DbBaseSql(_connection1, "")) {
                using (DbBaseSql db = new DbBaseSql(_connection, ""))
                {
                    ...
                    using (DataTable ddt = db.GetDataTable("select getdate() as currentdate,cast(cast(DATEADD(DAY, (DATEDIFF(DAY, 0, GETDATE()) / 7) * 7 + 7, 0) as Date)as varchar) + ' 09:00' AS NextMonday"))
                    {
                        foreach (DataRow row in ddt.Rows)
                        {
                    ...
                    */
                     ...
                     * */
                        ...
                        using (VirtualStorage cds = new VirtualStorage())
                        {
                            ...
                            lblBodyId.Text = "CHILDFORM." + _client + "." + formid;
                            cds.Register(ref mp, lblBodyId.Text, false, "", "", "QA"); //MONITOR.26.137
                        }
                        using (DataTable dtan = db1.GetDataTable("select '(Entire Team)' as [name],'_Team' as user_id UNION select lastnm + ', ' + firstnm as [name],us.user_id from usr us inner join usr_team ut on ut.user_id=us.user_id where ut.team_id in (select team_id from usr_team where user_id='" + lblAgent.Text + "') and us.status <>'2' order by [name] asc"))
                        {
                            Boolean first = true;
                            ...
                            {
                                if (dtan.Rows[i]["name"].ToString() != "")
                                {
                                    if (!first)
                                    {
                                        lblAgentTeamMembers.Text += "|";
                                    }
                                    first = false;
                                    lblAgentTeamMembers.Text += dtan.Rows[i]["user_id"] + "~" + dtan.Rows[i]["Name"];
                                }
                            }
                        }
                    ...
                    */
                        try
                        {
                            ...
                            String msql = "select lastnm,firstnm,user_id from usr usr where status='1' and role not in ('CSR','Admin') order by lastnm,firstnm,user_id";
                            //If collective-solution, restrict the managers to group & team leaders in the current project.
                            if (urlprefix().ToLower() == "collective-solution.")
                            {
                                msql = "select lastnm,firstnm,user_id from usr usr where status='1' and (user_id IN (select spvr_user_id from team where status<>'I' and project_id in (select project_id from usr_team ut inner join team tm on tm.team_id=ut.team_id and ut.user_id='" + db1.reap(lblAgent.Text) + "')) OR (user_id IN (select spvr_user_id from grp where status<>'I' and project_id in (select project_id from usr_team ut inner join team tm on tm.team_id=ut.team_id and ut.user_id='" + db1.reap(lblAgent.Text) + "'))))";
                            }
                            using (DataTable dtan = db1.GetDataTable(msql))
                            {
                                Boolean first = true;
                    ...
                    */
                    /*
                    using (DataTable dtan = db1.GetDataTable("select distinct client_dept from monitor_imprvmnt order by client_dept"))
                    {
                        Boolean first = true;
