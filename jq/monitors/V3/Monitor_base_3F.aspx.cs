...
public partial class _Monitor_base_3F : SitePage
{
    String _connection = "";
    String _connection1 = "";
    String _client = "0";
                ...
                using (DbBaseSql db = new DbBaseSql(_connection, ""))
                {
                    ...
                    using (DataTable dt = db.GetDataTable("select field,val from PRT where guid='" + uid + "'"))
                    {
                        ...
                        using (VirtualStorage cds = new VirtualStorage())
                        {
                            ...
                            //Can I do this twice?  This is to substitute the flow, which is at the client level.
                            cds.Register(ref mp, "MONITOR." + _client, false, "", "", "QA", urlprefix(true).ToLower()); //MONITOR.26.137
                    ...
                    */
                    ...
                    lblAgentTest.Text = "No";

                    if (urlprefix().ToLower() == "km2.")
                    {
                        if ((lblMode.Text == "update") && (lblAgent.Text.ToLower() == lblViewer.Text.ToLower()))
                        {
                            lblAgentTest.Text = "Yes";
                            lblAgentLockout.Text = "Yes"; //duck
                            using (DataTable dtkm2A = db1.GetDataTable("select dbo.getallformstatus('" + lblMonitorId.Text + "') as Status"))
                            {
                                if (dtkm2A.Rows.Count >= 1)
                                {
                                    String mystatus = dtkm2A.Rows[0]["Status"].ToString().ToLower();
                                    if ((mystatus == "complete") || (mystatus == "pending agent confirmation"))
                                    {
                                        lblAgentLockout.Text = "No";
                                    }
                                }
                            }
                        }
