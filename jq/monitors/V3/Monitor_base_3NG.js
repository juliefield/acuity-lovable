...
$(document).ready(function () {
    ...
    */
    ...
    var csf = {
        ...
        GROUP_SUBMIT: false //special logic when creating a new form.
    }
    ...
    $(".ac-submit").bind("click", function (e) {
        ...
        if ($(this).hasClass("ac-draft")) {
            if ((!$(".ac-islive").is(":checked")) || ((a$.urlprefix() == "walgreens.") && ($("#lblSqfcode").html() == "3"))) {
                $(".ac-islive").prop("checked", false);
                $(".qst-timestamp-islive").val("");
            }
        }
        else {
        ...
        */
        ...
        if (a$.urlprefix() == "km2.") {
            ...
            if ($("#lblSqfcode").html() == "3") { //Group Coaching Form
                CreateSidekickEntry = true;
            }
        }
            ...
            if ($("#lblSqfcode").html() == "4") { //SMART Planner
                CreateSidekickEntry = true;
            }
            if ($("#lblSqfcode").html() == "5") { //Performance Review
                CreateSidekickEntry = true;
            }
        }
        ...
        if ((csf.GROUP_SUBMIT) && ($("#lblMode").html() == "new")) {
            var foundone = false;
            var bld = "Journal records will be created for these agents:\n\n";
            var userids = [];
            var ubld = "";
            $("#monitoruserlistchart li").each(function () {
                if ($('input[type="checkbox"]', this).is(":checked")) {
                    foundone = true;
                    bld += $(".li-fullname", this).html() /*$(".li-firstnm", this).html() + " " + $(".li-lastnm", this).html()*/ + "\n";
                    userids.push($('input[type="checkbox"]', this).attr("userid"));
                    ubld += ((ubld != "") ? "," : "") + $('input[type="checkbox"]', this).attr("userid");
                }
            });
            if (!foundone) {
                alert("No agents selected (please select from the list)");
            }
            else {
                var wasdraft = false;
                if (!$(".ac-islive").is(":checked")) {
                    bld += "\nParticipants cannot be updated after saving as a draft.  To update, click Cancel, update participants and Save as Draft.";
                    wasdraft = true;
                }
                bld += "\nContinue?";
                if (confirm(bld)) {

                    $(".group-attendees-save").html(ubld);
                    data.mans = harvestQuestions();

                    //For safety, I'll just replicate the entire save and put a wrapper around it.
                    //I can try sequential or multi-thread to see which is best.
                    if (false) {
                        alert("Submitting group form (in development)...");
                    }
                    else {
                        $(".ac-submit,.ac-delete,.ac-draft,.ac-close").hide();
                        $(".ac-close").parent().append("Please wait...");
                        for (var u in userids) {
                            data.examinee = userids[u];
                            a$.ajax({
                                type: "POST",
                                service: "JScript",
                                async: false,
                    ...
                    if (true) { //(wasdraft) { //If cancelling distribution, ALWAYS reset it to draft.
                        $(".ac-islive").prop("checked", false);
                        $(".qst-timestamp-islive").val("");
                        $(".ac-delete").remove();
                        e.stopPropagation();
    ...
    function client_action_init() {
        ...
        if (($("#lblAgent").html() != $("#lblViewer").html()) || (!csf.RATE_JOURNAL)) {
            ...
            //TODO: duck - First check to be sure this journal hasn't already been rated (probably something in spinelib)
                    ...
                    for (var i in json.form.mans.answers) {
                        if (json.form.mans.answers[i].friendlyname == qst) {
                            ...
                            if ($(this).attr("type") == "checkbox") {
                                ...
                                if ((a$.urlprefix() == "walgreens.") && ($("#lblSqfcode").html() == "3")) {
                                    if (json.form.mans.answers[i].friendlyname == "GroupAttendees") {
                                        //alert("debug: loading group attendees");
                                        $(".group-attendees").html(json.form.mans.answers[i].answertext);
                                    }
                                }
                            }
                            break;
        ...
        if ($("#lblMode").html() == "new") {
            ...
            if (!$(".ac-islive").is(":checked")) {
                if (a$.urlprefix() == "walgreens.") {
                    $(".ac-save").attr("value", "Update/Submit");
                } else {
