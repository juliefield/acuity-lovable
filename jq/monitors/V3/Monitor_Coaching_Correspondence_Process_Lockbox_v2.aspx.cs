...
public partial class _Monitor_Coaching_Correspondence_Process_Lockbox_v2 : SitePage
{
    String _connection = "";
    String _connection1 = "";
    Boolean isnew = false;
        ...
        if (!IsPostBack) {
            using (DbBaseSql db1 = new DbBaseSql(_connection1, "")) {
                using (DbBaseSql db = new DbBaseSql(_connection, ""))
                {
                    ...
                    //     }
                    // }
                    using (DataTable ddt = db.GetDataTable("SELECT CONVERT(varchar, GETDATE(), 120) AS today, GETDATE() AS currentdate, CONVERT(varchar, DATEADD(DAY, (DATEDIFF(DAY, 0, GETDATE()) / 7) * 7 + 7, 0), 120) + ' 09:00' AS NextMonday"))
                    {
                        foreach (DataRow row in ddt.Rows)
                        {
                            lblCurrentDate.Text = row["currentdate"].ToString();
                            lblToday.Value = row["today"].ToString(); // Updated to include timestamp
                            //inputReleaseDate.Value = row["NextMonday"].ToString(); // Overwritten if this is an update.
                            inputReleaseDate.Value = row["today"].ToString(); // Overwritten if this is an update.
                            lblNextMonday.Value = row["NextMonday"].ToString();
                        }
    ...
    protected void rebuttalme_click(object sender, EventArgs e)
    {
        return; //DEBUG
    }
        protected void rebuttalmeb_click(object sender, EventArgs e)
    {
        return; //DEBUG
    }
        protected void rebuttalmec_click(object sender, EventArgs e)
    {
        return; //DEBUG
    }    
    
        protected void coachme_click(object sender, EventArgs e)
    {
        return; //DEBUG
    }       
