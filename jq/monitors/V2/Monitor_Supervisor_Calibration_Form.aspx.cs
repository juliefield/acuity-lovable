...
public partial class _Monitor_Supervisor_Calibration_Form : SitePage
{
    String _connection = "";
    String _connection1 = "";
    Boolean isnew = false;
            ...
            using (DbBaseSql db1 = new DbBaseSql(_connection1, ""))
            {
                using (DbBaseSql db = new DbBaseSql(_connection, ""))
                {
                    using (DataTable dt = db.GetDataTable("select field,val from PRT where guid='" + uid + "'"))
                    {
                        ...
                        {
                            selReviewType.Value = "Supervisor Calibration";
                        }
                            ...
                            foreach (DataRow rowup in dtup.Rows)
                            {
                                switch (rowup["question"].ToString())
                                {
                                    ...
                                    case "callpartytype":
                                        ...
                                        break;
                                    case "accountnum":
                                        inpAccountNum.Value = rowup["answer"].ToString();
                                        break;
                                    case "calltype":
            ...
            if (lblMode.Text == "new")
            {
                ...
                using (DataTable dtaux = db1.GetDataTable("select tm.team_id,tm.group_id,grp.loc_id from usr_team ut inner join team tm on tm.team_id=ut.team_id inner join grp on grp.group_id=tm.group_id where ut.user_id='" + lblAgent.Text + "'")) {
                    if (dtaux.Rows.Count > 0) {
                        ...
                        "(20/*HARDCODED*/,'" + lblAgent.Text + "','" + dtaux.Rows[0]["team_id"].ToString() + "','" + dtaux.Rows[0]["group_id"].ToString() + "','" + dtaux.Rows[0]["loc_id"].ToString() + "',2/*HARDCODED*/,'" + lblCallid.Text + "','" + lblCalldate.Text + "','" + lblCalldate.Text + "',100,0,100," + txtSuccessRate.Value.Replace("%","") + "," + txtSuccessRate.Value.Replace("%","") + ",getdate(),'" + lblSupervisor.Text + "');select scope_identity();"))
                        {
                        ...
                        //         // }
                        //         db1.Update("insert into KPI_RESP (kpi_id,mqf#,line#,level#,range,kpi_score,kpi_weight_factor,entdt,entby,project_id) values " +
                        //             "('" + dtkins.Rows[0][0].ToString() + "'," + qmqf + "/*HARDCODED*/,1,1,'" + kpi_score.ToString() + "','" + kpi_weight.ToString() + "',cast(floor(cast(getdate() as float)) as datetime),'" + lblSupervisor.Text + "',2/*HARDCODED*/)");
                        //     }
                        // }
                    }
                }
    ...
    protected void fillv2resp(String monitorid)
    {
        ...
        {
            db1.Update("delete from monitor_resp_v2prototype where monitor_id='" + monitorid + "'");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer,answertext) values ('" + monitorid + "',20/*HARDCODED*/,'Q1','" + txtQ1.Value + "','" + txtQ1text.Value + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer) values ('" + monitorid + "',20/*HARDCODED*/,'Comments','" + db1.reap(txtComments.Value) + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer) values ('" + monitorid + "',20/*HARDCODED*/,'reviewtype','" + db1.reap(txtReviewType.Value) + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer) values ('" + monitorid + "',20/*HARDCODED*/,'onlineguidance','" + db1.reap(txtOnlineGuidance.Value) + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer) values ('" + monitorid + "',20/*HARDCODED*/,'callpartytype','" + db1.reap(txtCallpartyType.Value) + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer) values ('" + monitorid + "',20/*HARDCODED*/,'Total','" + txtSuccessRate.Value.Replace("%","") + "')");
            // db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer) values ('" + monitorid + "',20/*HARDCODED*/,'AssuranceTotal','" + txtAssuranceTotal.Value.Replace("%", "") + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer) values ('" + monitorid + "',20/*HARDCODED*/,'accountnum','" + db1.reap(txtAccountNum.Value) + "')");
        }
