...
public partial class _Monitor_Unified_Quality_Assurance_Form : SitePage
{
    String _connection = "";
    String _connection1 = "";
    Boolean isnew = false;
                            ...
                            foreach (DataRow rowup in dtup.Rows)
                            {
                                switch (rowup["question"].ToString())
                                {
                                    ...
                                    case "Q32":
                                        ...
                                        break;
                                    case "Q33":
                                        txtQ33.Value = rowup["answer"].ToString();
                                        txtQ33text.Value = rowup["answertext"].ToString();
                                        break;
                                    case "Q1Comments":
                                    ...
                                    case "Q32Comments":
                                        ...
                                        break;
                                    case "Q33Comments":
                                        txtQ33Comments.Value = rowup["answer"].ToString();
                                        break;                                                                                                                                                                                                                                                                                                                                                                                                                                                        
                                    case "Comments":
                ...
                using (DataTable dtaux = db1.GetDataTable("select tm.team_id,tm.group_id,grp.loc_id from usr_team ut inner join team tm on tm.team_id=ut.team_id inner join grp on grp.group_id=tm.group_id where ut.user_id='" + lblAgent.Text + "'")) {
                    if (dtaux.Rows.Count > 0) {
                        ...
                        if (saveme)
                        {
                            ...
                            //BLUEGREEN EXCEPTION:  Store the monitors on the REVIEW DATE (for all others it's by call date (which actually should be revisited)
                            // WAS:
                            ...
                            //    "('" + lblAgent.Text + "','" + dtaux.Rows[0]["team_id"].ToString() + "','" + lblCalldate.Text.Split(' ')[0] + "',cast(floor(cast(getdate() as float)) as datetime),'" + lblSupervisor.Text + "',1/*HARDCODED*/,'" + monitorid + "');select scope_identity();"))
                            //{
                            //    db1.Update("insert into KPI_RESP (kpi_id,mqf#,line#,level#,range,kpi_score,kpi_weight_factor,entdt,entby,project_id) values " +
                            //        "('" + dtkins.Rows[0][0].ToString() + "',29/*HARDCODED*/,1,1,'" + txtSuccessRate.Value.Replace("%","") + "','" + kpi_score.ToString() + "','" + kpi_weight.ToString() + "',cast(floor(cast(getdate() as float)) as datetime),'" + lblSupervisor.Text + "',1/*HARDCODED*/)");
                            //}
                            using (DataTable dtkins = db1.GetDataTable("insert into KPI_HSTRY (user_id,team_id,respdt,entdt,entby,project_id,reference_id) values " +
    ...
    protected void fillv2resp(String monitorid)
    {
        using (DbBaseSql db1 = new DbBaseSql(_connection1, ""))
        {
            ...
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer,answertext) values ('" + monitorid + "',21/*HARDCODED*/,'Q30','" + txtQ30.Value + "','" + txtQ30text.Value + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer,answertext) values ('" + monitorid + "',21/*HARDCODED*/,'Q31','" + txtQ31.Value + "','" + txtQ31text.Value + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer,answertext) values ('" + monitorid + "',21/*HARDCODED*/,'Q32','" + txtQ32.Value + "','" + txtQ32text.Value + "')");
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer,answertext) values ('" + monitorid + "',21/*HARDCODED*/,'Q33','" + txtQ33.Value + "','" + txtQ33text.Value + "')");
            ...
            db1.Update("insert into monitor_resp_v2prototype (monitor_id,sqf_code,question,answer) values ('" + monitorid + "',21/*HARDCODED*/,'onlineguidance','" + db1.reap(txtOnlineGuidance.Value) + "')");
    ...
    protected void getscoreandweight(String raw,ref int kpi_score, ref double kpi_weight)
    {
        using (DbBaseSql db1 = new DbBaseSql(_connection1, ""))
        {
            using (DataTable dt = db1.GetDataTable("select score from kpi_scoring where mqf#=29/*HARDCODED*/ and " + raw + " >= range1low and " + raw + " <= range1high"))
            {
                kpi_score = Convert.ToInt32(dt.Rows[0][0]);
            }
            using (DataTable dt2 = db1.GetDataTable("select weight_factor from kpi_mqf where mqf#=29/*HARDCODED*/ and txt<>''"))
            {
