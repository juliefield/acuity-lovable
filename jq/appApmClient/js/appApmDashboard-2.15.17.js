...
(function () {
    ...
    var GAUGEBASIS = SCOREBASIS;
    if ((a$.urlprefix() == "united.") || (a$.urlprefix() == "united2.")) {
        GAUGEBASIS = 110;
    }

    window.SCOREBASIS = SCOREBASIS;
    window.SCORELOW = SCORELOW;
        ...
        function loaded(json) {
            if (a$.jsonerror(json)) { } else {
                ...
                if (dsh.hiredates) {
                    ...
                    $("#drpHiredates").data('dateRangePicker').setDateRange(dsh.hiredate.start, dsh.hiredate.end);
                    $("#selHiredates").trigger("liszt:updated"); //new
                    $("#selHiredates_chzn").hide();
                    $("#newhiredatesdl .hd-label").html(dsh.hiredate.start + " - " + dsh.hiredate.end);
    ...
    function initcontrols(opts) {
        if (((a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-make40.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix() == "ers.") || (a$.urlprefix() == "make40.")) && ($.cookie("TP1Role") == "Team Leader")) {
            peerevaluate = true;
        }
        if (opts) controlopts = opts; //use an "extend" when you make default parameters.
    ...
    function genchart(myop) {
        if (RANKTHEME) {
            if ((a$.urlprefix() == "cox.") || (a$.urlprefix() == "walgreens.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix() == "cox-test.") || (a$.urlprefix() == "cox-NOTmake40.") || (a$.urlprefix() == "v3.")) {
            }
            else {
            ...
            //}
            if (tenit) {
                if ((a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix() == "cox-test.") || (a$.urlprefix() == "cox-NOTmake40.") || (a$.urlprefix() == "v3.")) {
                }
                else if (a$.urlprefix() == "walgreens.") {
                    delete myop.yAxis[0].max;
                    delete myop.yAxis[0].min;
                }
                else {
                    myop.yAxis[0].max = SCOREBASIS;
                    if (a$.urlprefix() == "walgreens.") { //Note: This is currently bypassed by previous condition.
                        myop.yAxis[0].min = -3.0;
                        //myop.yAxis.type = "logarithmic";
                    }
                }
            }
            myop.yAxis[0].plotBands = [{
            ...
            if (a$.urlprefix() == "walgreens.") {
                myop.yAxis[0].plotBands[0] = { color: 'Black', from: -0.002, to: 0.00 }; //Defeat red line.
            }
        ...
        if (wh.length > 1) {
            ...
            for (var i in pars) {
                ...
                if (par.length > 1) {
                    ...
                    if (phrase.length > 1) {
                        if (phrase[0] == "init") {
                            $('#sel' + phrase[1] + 's').val(a$.xss(par[1])).trigger("liszt:updated");
                        }
                    }
                }
            }
    ...
    function viewparams(viewindex, eachisall) {
        ...
        var supsp = "".split(",");
        if (a$.exists(suppressboxlist)) {
            supsp = suppressboxlist.split(",");
        }
        ...
        //filterParams = $("#filtergroup").FilterAction({ action: "retrieve" });
        //alert("debug2:filterParams = " + filterParams);

        if (a$.exists(filterParams)) {
            if (filterParams != "") {
                if (!first) bld += "&";
                bld += filterParams;
            }
        ...
        //DEBUG TESTING DATERANGE
        //if (document.getElementById("rdoGrid").checked && (bld.indexOf("daterange") < 0) /* && (a$.urlprefix() == "ers-mnt.") */) {
        //    bld = bld + "&daterange=01/01/2019,08/31/2019";
        //}
        return bld;
    }
    ...
    function pagecontrolparams(includePayPeriods) {
        ...
        if (bld.indexOf("prefix=") < 0) {
            bld += "&prefix=" + a$.urlprefix().split(".")[0];
        }
        return bld;
    }
    ...
    function refreshboxes(obj) {
        ...
        if (document.getElementById("StgScorecard") != null) {
            urlbld += "&scorecard=" + $("#StgScorecard select").val()
        }
        //if (document.getElementById("StgNoDateRestriction") != null) {
        //    if ($("#StgNoDateRestriction select").val() == "On") {
        if (a$.urlprefix() == "chime.") {
            urlbld += "&nodaterestriction="; //"" means restrict dates to 1 year
        }
        else {
        ...
        } else if ((a$.urlprefix() == "collective-solution.") || (a$.urlprefix() == "sprintgame.") || (a$.urlprefix(true).indexOf("mnt") == 0)) { //2016-07-22 = Added for managers, to restrict by location //This is going in as a CLUGUE to avoid a complete visitation of the boxfilter system.
        ...
        $.ajax({
            ...
            error: function (request, textStatus, errorThrown) {
                ...
                //SUPPRESS:  alert('NETWORK ERROR\n\nID:1701 - Please check your connection.');
                console.log('NETWORK ERROR\n\nID:1701 - Please check your connection.');
                $(".err-content", a$.WindowTop().document).html('NETWORK ERROR:' + request.status + ' / ' + textStatus + ' / ' + errorThrown + '<br /><br />ID:1701 - Please check your connection.');
                $(".err-icon", a$.WindowTop().document).show();
            },
        ...
        function loadCombosFromXML(xml) {
            ...
            for (myi = 0; myi < whichsplit.length; myi++) {
                ...
                loadComboFromXML(xml, c,
                    (c == 'Qualityform') || ((c != 'Payperiod') && ((c != 'Project') || (((dbd == "Source") || (dbd == "Attrition") || (fil == "Agent_Attrition") || (fil == "Reports")) && (($.cookie("TP1Role") == "Admin") || ($.cookie("TP1Role") == "CorpAdmin") || ($.cookie("TP1Role") == "Management")))) && (c != 'Xaxis') && (c != 'DataSource') && (c != 'ACDDate') && (c != 'Trendby')), //all at top
                /*(c == 'Payperiod')*/
                    (a$.urlprefix() != "performant.") &&
                    (a$.urlprefix() != "act.") &&
                    (a$.urlprefix() != "rcm.") &&
                    (a$.gup("oldfilters") != "Y") && (c == 'Project') && (($.cookie("TP1Role") == "Admin") || ($.cookie("TP1Role") == "CorpAdmin") || ($.cookie("TP1Role") == "Management")), //all at bottom
                    ((c != 'Project') || (((dbd == "Source") || (dbd == "Attrition") || (fil == "Agent_Attrition") || (fil == "Reports")) && (($.cookie("TP1Role") == "Admin") || ($.cookie("TP1Role") == "CorpAdmin") || ($.cookie("TP1Role") == "Management")))) && (c != 'Payperiod') /* && (c != 'Agency') && (c != 'Agencyoffice') */ && (c != 'Xaxis') && (c != 'DataSource') && (c != 'ACDDate') && (c != 'Trendby') && (c != 'Qualityform'), //each at top
                    (a$.urlprefix() != "performant.") &&
                    (a$.urlprefix() != "act.") &&
                    (a$.urlprefix() != "rcm.") &&
                    (a$.gup("oldfilters") != "Y") && (c == 'Project') && (($.cookie("TP1Role") == "Admin") || ($.cookie("TP1Role") == "CorpAdmin") || ($.cookie("TP1Role") == "Management")), //each at bottom
                ...
                if (c == "Payperiod") {
                    $('#selPayperiod2s').empty().append($("#selPayperiods").html().toString());
                    $('#selPayperiod2s').trigger("liszt:updated");
                }
                if (c == "CSR") {
                    //alert("debug: CSR load confirmed");
                    $("#loader_filters").html("CONFIRMED");
                }
            }
            try {
            ...
            splitdates(document.getElementById('selPayperiod2s'), document.getElementById('spanDatefrom2'), document.getElementById('spanDateto2'));
            if (qTeamRefresh) {
                refreshboxes({
                    which: 'Team,CSR',
                    success: function () { }
    ...
    function loadComboFromXML(xml, which, allattop, allatbottom, eachattop, eachatbottom, eachpayperiodatbottom, eachmonthatbottom, settoblank) {
        var ae = true;
        if ($.cookie("TP1Boxfilter") == "on") {
            if ((which != "KPI") && (which != "SubKPI") && (which != "Qualityform")) {
                if (($.cookie("TP1Role") == "Group Leader") && ((which == "CSR") || (which == "Team"))) { }
                else if (($.cookie("TP1Role") == "Team Leader") && (which == "CSR")) { }
                else if (($.cookie("TP1Role") == "Team Leader") && (which == "Team") && (suprestrict != "Y")) { }
                else if (($.cookie("TP1Role") == "CSR") && ((which == "CSR") && (suprestrict != "Y"))) { }
                else if (false) { //(peerevaluate && ($.cookie("TP1Role") == "Team Leader") && ((which == "Group") || (which == "Team"))) { //NOTE: peerevaluate
                }
                else {
                    ae = false;
                }
        ...
        try {
            ...
            $(xml).find(which + 's').each(function () {
                ...
                $(this).find(which).each(function () {
                    ...
                    if (addit) {
                        if ($(this).find("key").text() == "disabled") {
                            ...
                            if (which == "CSR") {
                                if ((a$.urlprefix() == "chime.") || (a$.urlprefix().indexOf("chime-make40") >= 0) || (a$.urlprefix() == "chime-mnt.")) {
                                    //Defeated: chimeNoInTraining = true;
                                }
                            }
                ...
                if (ae) {
                    ...
                    if (sb.options != null) {
                        if (sb.options.length > 0) {
                            if (($("#StgSupervisorFilters select").val() == "Entire Project") || ((a$.urlprefix() == "walgreens.") && ($("#StgSupervisorFilters select").val() == "LOB"))) {
                ...
                if (which == "Trendby") {
                    if (!((a$.urlprefix() == "cox.") || (a$.urlprefix() == "ers.") || (a$.urlprefix() == "veyo.") || (a$.urlprefix() == "mtm.") || (a$.urlprefix() == "km2.") || (a$.urlprefix() == "test-km2.") || (a$.urlprefix() == "walgreens.") || (a$.urlprefix() == "ultracx."))) {
                ...
                else if ((which == "CSR") && ($.cookie("TP1Role") == "CSR")) {
                    if (sb.options.length > 1) {
                        a$.addOption(sb, "(All)", "");
                        a$.addOption(sb, "(Each)", "each");
                        $("#selCSRs").val($.cookie("TP1Username"));
                        ininit = false;
                        settoblank = false;
                    }
                }
            ...
            if (which == "Project") {
                if ((a$.urlprefix() == "chime.")) {
                    function chimepatabset() {
                        if ($.cookie("TP1Role") == "Admin") {
                            if ($("#selProjects option:selected").text().indexOf("Alight") >= 0) {
                                $("#perfectattendancetab").show();
                                if ($("#PerfectattendanceIframe").attr("src") == "") {
                                    $("#PerfectattendanceIframe").attr("src", "../3/ng/AgameFlex/ChimePerfectAttendance.aspx?prefix=" + a$.urlprefix().split(".")[0]).show();
                                }
                            }
                            else {
                                $("#perfectattendancetab").hide();
                            }
                        }
                    }
                    $("#selProjects").bind("change", function () {
                        chimepatabset();
                    });
                    chimepatabset();
                }
    ...
    var plotme = function (viewindex, reset, supresstools) {
        ...
        */
        ...
        if ((a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-make40.") || (a$.urlprefix() == "cox-mnt.")) {
            if ($.cookie("CoxSkipWhen2") != "") {
                $.cookie("CoxSkipWhen2", "" + (parseInt($.cookie("CoxSkipWhen2"), 10) + 1));
                if ($.cookie("CoxSkipWhen2") == "2") {
                    return;
                }
            }
        }

        if (($("#selSubKPIs > option").length < 2) || ($("#selSubKPIs").val() == "") || $("#rdoTrend").is(":checked")) {
            $('#graphsubtab').hide();
            subgraph = false;
        } else {
            subgraph = true;
    ...
    var plotme_main = function (viewindex, reset, supresstools, subgraph, subparams) {
        ...
        if (subgraph) {
            ...
            if (location.hash != "#Messaging") { //Added 2017-05-30 - to keep the home graph tab from showing when called from notification app.
                ...
                if (!$("#graphtab").eq(0).hasClass("ui-state-active")) {
                    if (window.AlreadyUp) {
                        $('#graphlabel').trigger('click'); //On initial load, we'll never be starting on a subgraph.  It's                     
                    }
                    window.AlreadyUp = true;
                }
                $("#graphsubtab").hide();
            }
        }
        ...
        if (reset) {
            if (document.getElementById("rdoGrid").checked && a$.exists(op.report)) { //TODO: There remnants are for the split report, which will be changed before implementation
                ...
                a$.ajax({
                    ...
                    cache: false,
                    error: a$.ajaxerror,
                    params: viewparams(viewindex, false)
                ...
                function loaded(json) {
                    ...
                    if (a$.jsonerror(json)) { } else {
                        ...
                        if (json.report.show) {
                            ...
                            $("#" + rr + " .rpt-filters").hide();
                            $("#" + rr + " .rpt-title").css("padding", "10px");
                            $("#" + rr + " .rpt-table-topper").css("padding", "0px").css("text-align", "left").css("margin-top", "40px");
                            ...
                            $(".acuity-download").unbind().bind("click", function () {
                                ...
                                acuitytable_download("csv", $(this).parent().parent().children().eq($(this).parent().index() + 1), title);
                            });
                ...
                if (a$.exists(op.report)) {
                    ...
                    var rr = ($("#reporttab").eq(0).hasClass("ui-state-active")) ? "myreportreportcontainer" : op.report.renderTo;
                    $("#" + rr).html("").hide();
                    if ((a$.urlprefix() == "km2.") || (a$.urlprefix() == "km2-make40.") || (a$.urlprefix() == "test-km2.")) {
        ...
        try {
            ...
            if (hidechart) {
                ...
                if (chartdiv) {
                    ...
                    chartdiv.style.width = ($(window).width() - (325 + reportwidth)) + 'px';
                    chartdiv.style.height = ($(window).height() - 140) + 'px';
                }
            }
            var tabsdiv = document.getElementById("tabs");
            if (tabsdiv) {
                tabsdiv.style.width = ($(window).width() - 288) + 'px';
            }
            var pgdiv = document.getElementById('mytable1');
            if (pgdiv) {
                pgdiv.style.width = ($(window).width() - 278) + 'px';
        ...
        if ($("#graphtab").hasClass("ui-tabs-selected")) {
            if (($("#selProjects").val() == "") || ($("#selProjects").val() == "each")) {
                a$.hideprogress("plotprogress");
                if (!ORIGINALPASS) {
                    alert("Hello, we've upgraded the filters to be more user-friendly!\nThe filters are now maintained when switching tabs,\nand multiple selections are now supported more fully.\n\nTo view a chart on the 'Home' tab, however,\nyou'll need to first select a project from the filters.\n\nProject = (All) or (Each) are valid selections on the 'Reports' tab, but are not supported on the 'Home' tab.");
                }
                ORIGINALPASS = false;
                return;
            }
        ...
        if (document.getElementById("StgScorecard") != null) {
            urlhld += "&scorecard=" + $("#StgScorecard select").val();
        }
        if (document.getElementById("StgInTraining") != null) {
            urlhld += "&InTraining=" + $("#StgInTraining select").val();
        }
        if (document.getElementById("StgAgentStatus") != null) {
            urlhld += "&AgentStatus=" + $("#StgAgentStatus select").val();
        }

        if (urlhld.toString().indexOf("role=") < 0) {
            urlhld += "&role=" + $.cookie("TP1Role");
        }
        ...
        if (document.getElementById("StgDisplayScreen") != null) {
            urlhld += "&DisplayScreen=" + $("#StgDisplayScreen select").val();
        }
        ...
        $.ajax({
            ...
            error: function (request, textStatus, errorThrown) {
                //alert('Error loading ListConfig XML Document:' + request.status + ' / ' + textStatus + ' / ' + errorThrown);
                //SUPPRESS: alert('NETWORK ERROR\n\nID:1702 - Please check your connection.');
                console.log('NETWORK ERROR\n\nID:1702 - Please check your connection.');
                $(".err-content", a$.WindowTop().document).html('NETWORK ERROR:' + request.status + ' / ' + textStatus + ' / ' + errorThrown + '<br /><br />ID:1702 - Please check your connection.');
                $(".err-icon", a$.WindowTop().document).show();
            },
        ...
        function loadChartFromXML(xml) {
            ...
            if ((dataset != "Pay") && ($("#selKPIs").val() == "")) {
                ...
                a$.showprogress("gaugeprogress1");
                a$.showprogress("gaugeprogress2");
                ...
                $.ajax({
                    ...
                    error: function (request, textStatus, errorThrown) {
                        //alert('Error loading ListConfig XML Document:' + request.status + ' / ' + textStatus + ' / ' + errorThrown);
                        //SUPPRESS: // alert('NETWORK ERROR\n\nID:1703 - Please check your connection.');
                        console.log('NETWORK ERROR\n\nID:1703 - Please check your connection.');
                        $(".err-content", a$.WindowTop().document).html('NETWORK ERROR:' + request.status + ' / ' + textStatus + ' / ' + errorThrown + '<br /><br />ID:1703 - Please check your connection.');
                        $(".err-icon", a$.WindowTop().document).show();
                ...
                for (var i = 0; i < 16; i++) $("#n2eedle" + i).rotate({
                    animateTo: -2
                });
    ...
    function loadme2(xml) {
        ...
        loadNeedlesFromXML(xml, plothold);
        a$.hideprogress("gaugeprogress1");
        a$.hideprogress("gaugeprogress2");
    }
    ...
    function loadChartOverlay(o) { //o.xml, o.overlay, o.raw, o.showLegend
        ...
        */
                    ...
                    if (!allblank) {
                        ...
                        $(this).find('Spec').each(function () {
                            ...
                            if ($.cookie("TP1Role") == "Team Leader") {
                                if ($("#selTeams").val() == "each") {
                                    candidate = true;
                                    nm = "Team";
                                    val = $.cookie("DefaultTeam");
                                }
                            }
                                ...
                                if ($("#selCSRs").val() == "each") {
                                    candidate = true;
                                    nm = "CSR";
                                    val = $.cookie("TP1Username").toUpperCase();
                                }
                            }
                            if (candidate) {
                                var sp = seriesparams.split("&");
                                for (var i in sp) {
                                    var sps = sp[i].split("=");
                                    if (sps[0] == nm) {
                                        if (sps.length > 1) {
                                            if (sps[1].toUpperCase() == val) {
                                                highlightme = true;
                                            }
                                        }
                                    }
                            ...
                            if ((a$.urlprefix() == "ers.") || (a$.urlprefix() == "make40.")) {
                                if (x.indexOf("Utilization A-GAME") >= 0) {
                                    if (($.cookie("TP1Role") != "CSR") && ($.cookie("TP1Role") != "Team Leader")) {
                                        //REMOVED 2018-02-06: except = true;
                                    }
                                }
                            }
                            ...
                            if ((fmt.indexOf("-X") < 0) || ($("#selKPIs").val() != "each") || except) {
                                ...
                                try {
                                    if (op.apmQid == "PayChart") {
                                        ...
                                        if (raw != "") {
                                            ...
                                            if (true) { //((a$.urlprefix() == "km2.") || (a$.urlprefix() == "veyo.") || (a$.urlprefix() == "veyo-mnt.")  || (a$.urlprefix() == "chime.") || (a$.urlprefix() == "chime-mnt.") || (a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix() == "cox-NOTmake40.")) { // || (a$.urlprefix() == "ers.") || (a$.urlprefix() == "make40.")) {
                                                ...
                                                if (true) { //}($("#StgScoreModel select").val() == "Raw") {
                                                    if ($("#selKPIs").val() != "") {
                                                        ...
                                                        a$.ajax({
                                                            ...
                                                            cache: false,
                                                            error: a$.ajaxerror,
                                                            params: $(this).find("key").text() + seriesparams
                                                                + ((document.getElementById("StgScorecard") != null) ? "&scorecard=" + $("#StgScorecard select").val() : "")
                                                                + ((filterParams != "") ? "&" : "") + filterParams, //getdrillparams(0 /*viewindex */ , seriesname, x), //this.series.name, this.point.category),
                                                            success: loaded
                                                        });
                                ...
                                for (i = 0; i < op.xAxis.categories.length; i++) {
                                    if (op.xAxis.categories[i] == x) {
                                        ...
                                        if (!((a$.urlprefix() == "km2.") || (a$.urlprefix() == "da.") || (a$.urlprefix() == "km2-make40.") || (a$.urlprefix() == "test-km2."))) {
                                        ...
                                        if (o.raw && (raw != "")) {
                                            ...
                                            if (!isNaN(parseFloat(series[i].formy))) {
                                                series[i].y = parseFloat(series[i].formy);
                                            }
                                            else {
                                            ...
                                            if (!isNaN(raw)) {
                                                ...
                                                if (maxy == null) {
                                                    ...
                                                    maxy = Math.max(maxy, Math.ceil(rawf));
                                                }
                                            }
                                            ...
                                            if ((a$.urlprefix() == "km2.") || (a$.urlprefix() == "km2-make40.") || (a$.urlprefix() == "test-km2.") || (a$.urlprefix() == "ers.") || (a$.urlprefix() == "make40.") || (a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix() == "cox-NOTmake40.")) {
                                                if ((raw != "") && (raw != "null")) {
                                                    series[i].formy = formatraw(raw, fmt);
                                                    series[i].rawfmt = fmt;
                                                } else {
                                        ...
                                        if (op.apmQid == "PayChart") {
                                            ...
                                            if (sourceAttrit) {
                                                ...
                                                if (o.bothScores) {
                                                    ...
                                                    if (o.raw) {
                                                        if (raw != "") {
                                                            if (!((a$.urlprefix() == "km2.") || (a$.urlprefix() == "km2-make40.") || (a$.urlprefix() == "test-km2.") || (a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix() == "cox-NOTmake40."))) {
                                        ...
                                        //if (rank != "") {
                                        series[i].name += "~" + formatrank(rank, false);
                                        //}
                                        if (RANKTHEME) {
                                            var rs = rank.split("/");
                                            if (parseInt(rs[2]) > 0) {
        ...
        if (op.apmQid == "PayChart") {
            ...
            op.tooltip.formatter = function () {
                var nm = this.point.name.split("~");
                var bld = '<div style="z-index:1000;padding:20px;margin:-50px;background-color:white;">'; //There's a CSS battle raging here.
            ...
            if ((a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix() == "cox-test.") || (a$.urlprefix() == "cox-NOTmake40.")) {
            ...
            op.plotOptions.series.dataLabels.formatter = function () {
                ...
                var score;
                if (a$.exists(this.point) && ((a$.urlprefix() == "km2.") || (a$.urlprefix() == "km2-make40.") || (a$.urlprefix() == "test-km2.") || (a$.urlprefix() == "ers.") || (a$.urlprefix() == "make40.") || (a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix() == "cox-NOTmake40."))) {
                ...
                if (ribbon) {
                    //var ts = '<div style="position:relative;overflow:visible;">' + score + '<div style="position:absolute;top: 20px;right:0px;"><img src="appApmClient/themes/default/images/blue-ribbons.gif" style="width:40px;"></img></div></div>';
                    var ts = score + '<div style="position:absolute;top: 60px;right:0px;"><img src="appApmClient/themes/default/images/blue-ribbons.gif" style="width:40px;"></img></div>';
                    return (ts);
                } else {
                    return (score);
                }
            ...
            op.tooltip.formatter = function () {
                ...
                if (pay) { //(this.series.version == 1) { //This DOESN'T WORK!  version is undefined, only name survives.
                    ...
                    function loaded(json) {
                        //if (a$.jsonerror(json)) { } else {
                        if (json.errormessage) { } else { //MAKEDEV (Tooltips shouldn't produce error messages, they should just not display the tip.).
                            //alert("debug: json.tooltip = " + json.tooltip);
                    ...
                    if (trend) {
                        ...
                        if (true) { //((a$.urlprefix() == "united.") || (a$.urlprefix() == "united.")) {
                            if (this.series.name.indexOf(" - ") > 0) {
                                bld += this.series.name.substring(0, this.series.name.indexOf(" - ") + 1) + '<br/>';
                            }
                            else {
                                bld += this.series.name + '<br/>';
                            }
                    ...
                    switch (tooltip.placement) {
                        ...
                        case "A":
                            bld += tooltip.html;
                            if (a$.urlprefix() == "collective-solution.") {
                                if ((tooltip.goal) && (tooltip.goal != "")) {
                                    bld = bld.replace("</b><br />", " of " + tooltip.goal + "</b><br />");
                                }
                            }
                            break;
                        case "R":
    ...
    function loadNeedlesFromXML(xml, plothold) {
        ...
        */
        ...
        if (true) {
                ...
                $(xml).find('Series' + s).each(function () {
                    ...
                    if (!allblank) {
                        ...
                        $(this).find('Point').each(function () {
                            ...
                            y = $(this).find("y").text();
                            if (a$.urlprefix() == "da.") {
                                oby = $(this).find("oby").text();
                                iby = $(this).find("iby").text();
                                lgy = $(this).find("lgy").text();
                                sly = $(this).find("sly").text();
                                hvcy = $(this).find("hvcy").text();
                                hgvy = $(this).find("hgvy").text();
                            }
                            var rank = "";
                            try {
                                rank = $(this).find("rank").text();
                                ...
                                if (s == 0) { //only 1 rank shown for now
                                    ...
                                    var rnk = formatrank(rank, true);
                                    $("#needlerank1,#needlerank2").html(rnk);
                                    if (rank == "") {
                                        $("#rankdiv1,#rankdiv2").hide();
                                    } else {
                                        if ($.cookie("ApmGuatModuleLoc") != "") { //LEAVE!
                                            if ($("#selLocations").val() == $.cookie("ApmGuatModuleLoc")) {
                                                ...
                                                controlopts.varirankgauge({
                                                    ...
                                                    ranges2: controlopts.rankRangesGUATEMALABONUS, //CHANGED: 2017-10-21, Guatemala changed bonus to be based on ranking (NONPERFORMACE changed to LEGACY)
                                                    paper2: controlopts.rank2_gpaper2
                                                }, 2);
                                                $("#rankdiv1,#rankdiv2").show();
                                            } else {
                                                var legacy = false;
                            ...
                            if (a$.urlprefix() == "walgreens.") {
                                $("#needle" + plothold).rotate({
                                    animateTo: 180 * (((parseFloat(y) + 3.0) / 2.0) / GAUGEBASIS)
                                });
                                $("#n2eedle" + plothold).rotate({
                                    animateTo: 180 * (((parseFloat(y) + 3.0) / 2.0) / GAUGEBASIS)
                                });
                            }
                            else {
                                $("#needle" + plothold).rotate({
                                    animateTo: 180 * (y / GAUGEBASIS)
                                });
                                $("#n2eedle" + plothold).rotate({
                                    animateTo: 180 * (y / GAUGEBASIS)
                                });
                            ...
                            if (plothold == 0) {
                                $("#gaugescore1,#gaugescore2").html(Math.round(y * 100.0) / 100.0);
                                if (a$.urlprefix() == "da.") {
                                    var bld = "<br />";
                                    if (!((oby == "0") || (oby == ""))) {
                                        bld += "OB: " + (Math.round(oby * 100.0) / 100.0);
                                        bld += "     ";
                                    }
                                    if (!((iby == "0") || (iby == ""))) {
                                        bld += "IB: " + (Math.round(iby * 100.0) / 100.0);
                                        bld += "     ";
                                    }
                                    if (!((lgy == "0") || (lgy == ""))) {
                                        bld += "CR: " + (Math.round(lgy * 100.0) / 100.0); //2025-01-19, was "LG:", I just changed the label.
                                        bld += "     ";
                                    }
                                    if (!((sly == "0") || (sly == ""))) {
                                        bld += "SL: " + (Math.round(sly * 100.0) / 100.0);
                                    }
                                    if (!((hvcy == "0") || (hvcy == ""))) {
                                        bld += "HVC: " + (Math.round(hvcy * 100.0) / 100.0);
                                        bld += "     ";
                                    }
                                    if (!((hgvy == "0") || (hgvy == ""))) {
                                        bld += "HGV: " + (Math.round(hgvy * 100.0) / 100.0);
                                    }
                                    $("#gaugescore1,#gaugescore2").css("top", "0px").html($("#gaugescore1").html() + bld);
                                }
                                $("#gaugelabel1,#gaugelabel2").html(((a$.urlprefix() == "chime.") || (a$.urlprefix().indexOf("chime-make40") >= 0) || (a$.urlprefix() == "chime-mnt.")) ? "Chime Score" : "Balanced Score");
                            } else {
                                $("#gaugescore1,#gaugescore2").html("");
                                $("#gaugelabel1,#gaugelabel2").html(((a$.urlprefix() == "chime.") || (a$.urlprefix().indexOf("chime-make40") >= 0) || (a$.urlprefix() == "chime-mnt.")) ? "Chime Scores" : "Balanced Score");
                                $("#rankdiv1,#rankdiv2").hide();
                            }
            ...
            for (var i = plothold; i < 16; i++) $("#n2eedle" + i).rotate({
                animateTo: -2
            });
        }
    ...
    function getdrillparams(viewindex, myname, mycategory) {
        ...
        var wrd = "";
        //MAKEDEV - Added 2018-05-22
        if (document.getElementById("rdoGrid").checked) {
            wrd = "Table Cell";
        }
        else if (document.getElementById("rdoBase").checked) {
            wrd = "Bar";
        }
    ...
    function graphtotable(viewindex) {
        ...
        */
        ...
        try {
            if (op.apmTouchpointDashboardFormatting) {
                var tl = document.getElementById('mytablelabel');
    ...
    function maketable(params, source) {
        ...
        if (document.getElementById("StgAgentStatus") != null) {
            if (urlhld.indexOf("&AgentStatus=") < 0) urlhld += "&AgentStatus=" + $("#StgAgentStatus select").val();
        }
        urlhld += "&role=" + $.cookie("TP1Role");

        if (document.getElementById("StgScorecard") != null) {
            if (urlhld.indexOf("&scorecard=") < 0) urlhld += "&scorecard=" + $("#StgScorecard select").val();
        }
        if (document.getElementById("StgDashboard") != null) {
        ...
        //Defeating cache for all old-style reports (nothing but trouble)
        var mur = a$.xss(window.location.pathname);
        if (mur.substring(mur.lastIndexOf('/') + 1).toLowerCase() == "reporttable.aspx") {
            cacheme = false;
        }
                    ...
                    for (n = 0; n < schemanames.length; n++) {
                        ...
                        if ((opt.colModel[n].name == "UserId") || (opt.colModel[n].name == "KPInum") || (opt.colModel[n].name == "SUBKPInum") || (opt.colModel[n].name == "fmt")) {
                            ...
                            if (hasallpercentageformatting) {
                                ...
                                opt.colModel[n].align = "center";
                            } else if (true) { //(!hasrawformatting) { //MAKEDEV  Changed 2018-03-08
                                opt.colModel[n].sorttype = "float";
                                opt.colModel[n].formatter = "number";
                                opt.colModel[n].align = "left"; //It's aligned left simply because of the table scrolling.
                            }
                        }
    ...
    function rendertable(opt, dvcode, params, source, schemanames) {
        if (opt.apmTableType) {
            ...
            if (1) { //
                ...
                $("#tablemessage").hide();
                $('#tabletab').show();
                if ((a$.urlprefix() == "walgreens.") && (($.cookie("TP1Role") == "Group Leader") || ($.cookie("TP1Role") == "Team Leader") || ($.cookie("TP1Role") == "CSR"))) {
                    //alert("debug: drill intercept");
                    $('#tabletab').hide();
                }
                else {
    ...
    function rowselected(recordnum) {
        ...
        function loaded(json) {
            ...
            if (a$.jsonerror(json)) { } else {
                var cnt = 0;
                /* Hijacking serverside_rowdv for a content block. */
                if ((json.dv.text != "") || (json.table.length > 0)) {
                    $("#rowdv_show").show();
                ...
                $("#rowdv_show").show();
                $("#serverside_rowdv").html(json.dv.text);
                var panelcid = "ROWDV." + a$.urlprefix() + (a$.exists(rd[index].KPInum) ? rd[index].KPInum : 0) /* + "." + (a$.exists(rd[index].SUBKPInum) ? rd[index].SUBKPInum : 0) */;
                $("#rowdviframe").show().css("height", "0px").attr("src", "../3/ng/ContentEditor/default.aspx?panelcid=" + panelcid + "&idself=rowdviframe&prefix=" + a$.urlprefix().split(".")[0]).show();
    ...
    function jqgrid_download(downloadtype, tid) {
        ...
        for (i = 0; i < opt.data.length; i++) {
            ...
            for (j = 0; j < colNames.length; j++) {
                if (!first) html += "\t";
                if (typeof data[colNames[j]] == "string") {
                    html += data[colNames[j]].replace(/[\n\r\t]/g, " "); // output each column as tab delimited MADEDEV
                }
                else {
                    html += data[colNames[j]]; // output each column as tab delimited MADEDEV
                }
                first = false;
            }
            html = html + "\n"; // output each row with end of line
    ...
    //TODO: Hook this up generally to tables.
            ...
            $("thead tr:nth-child(" + ($("tr", $("thead", sel).eq(0)).length + i) + ") th", sel).each(function () {
                if (($(this).is(":visible")) && ($(this).css('display') !== 'none')) {
                    ...
                    if ($(this).css('color') == 'rgb(255, 255, 255)') {
                        ...
                        if (mte == "") { //Maybe it's an anchor or input
                            if ($(" a", this).length > 0) {
                                mte = $(" a", this).html();
                            }
    ...
    function movefilters(viewindex) {
        ...
        for (var j in controlopts.views[vi].filters) {
            ...
            if (controlopts.views[vi].filters[j].customdate) {
                ...
                //Hide the select box
                //MAKEDEV
                var hidecustomdate = false; // ((a$.urlprefix() == "chime.") || (a$.urlprefix() == "chime-test.") || (a$.urlprefix() == "chime-make40.") || (a$.urlprefix() == "chime-mnt."));
                if (!hidecustomdate) { //MAKEDEV
                    //$("#sel" + id + "s").hide();
                    if (!$("#drp" + id + "s").length) {
                        //$("#drp" + id + "s").remove();
                    ...
                    $(".app-drp-" + id).remove();
                    $("#sel" + id + "s").parent().append('<span class="app-drp app-drp-' + id + '" id="' + 'drp' + id + 's' + '" name="' + 'drp' + id + 's' + '">&nbsp;</span>');
                    var dbf = false;
                    var dbleft = 0;
                    var dbtop = 0;
                    //if (controlopts.views[vi].filters[j].dashboardformatting) {
                    $(".app-drp-" + id).css("line-height", "24px");
                    dbf = true;
                    dbleft = 25;
                    dbtop = -250;
                    //}
                    function makedate(drpid) {
                        $("#drp" + id + "s").dateRangePicker({
                            ...
                            setValue: function (s) {
                                ...
                                if (drpid == "selHiredates") {
                                    //$("#newhiredatesdl .hd-label").html(cs[0] + " - " + cs[1]);
                                    if (s.indexOf(" to ") > 0) {
                                        dsh.hiredate.start = s.split(" to ")[0];
                                        dsh.hiredate.end = s.split(" to ")[1];
                                    }
                                    $("#newhiredatesdl .hd-label").html(s.replace(" to ", " - "));
                            ...
                            if (dbf) {
                                if (firstdbf) {
                                    // Don't Move Left? $(".date-picker-wrapper").css("left", (parseInt($(".date-picker-wrapper").css("left").split("p")[0]) + dbleft) + "px");
                                    // Don't Move Down? $(".date-picker-wrapper").css("top", (parseInt($(".date-picker-wrapper").css("top").split("p")[0]) + dbtop) + "px");
                                    firstdbf = false;
                                }
    ...
    function hidegauge() {
        $("#gaugediv1,#gaugediv2").animate({
            height: "0px"
        }, 1000);
        $("#gaugecollapse1,#gaugecollapse2").removeClass("icon-minus").addClass("icon-plus");
        $("#gaugescore1,#gaugescore2").html("Gauge Not Applicable");
        $("#gaugescore1,#gaugescore2").css("color", "gray");
        $("#rankdiv1,#rankdiv2").hide();
    }
    ...
    function showgauge() {
        ...
        $("#gaugecollapse1,#gaugecollapse2").removeClass("icon-plus").addClass("icon-minus");
        $("#gaugescore1,#gaugescore2").css("color", "white");
    }
    ...
    function setClientLabels() {
        ...
        //Sunpro is the only one on right now.
        if ((a$.urlprefix() == "sunpro.")) {
            $(".stats-blocks").show();
        }
        else if (a$.gup("stats") != "") {
        ...
        if ((a$.urlprefix() == "act.") || (a$.urlprefix() == "sunpro.") || (a$.urlprefix() == "collective-solution.") || (a$.urlprefix() == "walgreens.")) {
            //TODO: Get the first name using "getMe"
            a$.ajax({
                type: "GET",
                service: "JScript",
                async: true,
                ...
                success: function (json) {
                    if (a$.jsonerror(json)) { } else {
                        $("#header_userID_lbl").html(json.me.person.firstnm);
                    }
                }
            });
        }
        ...
        if ((a$.urlprefix() == "performant.") || (a$.urlprefix() == "performant-mnt.")) {
            $(".csrname").html("Case Worker");
            $(".sup-stgtoolslevel > .stg-steps > span").eq(0).html("Case Worker");
            $(".sup-stgtoolslevel > select > option").eq(0).html("Case Worker Level");
        }
        ...
        if (a$.urlprefix() == "walgreens.") {
            $(".scaname").html("LOB");
            $(".scaname-override").attr("value", "LOB");  //"Entire Project" is a key, "Entire LOB" sounds weird.
            $(".grpname").html("Supervisor");
            $(".teamname").html("FL");
            $(".csrname").html("TM");
            $("#csrlabel").html("TM");
            $(".sup-stgtoolslevel > .stg-steps > span").eq(0).html("Team Member");
            $(".sup-stgtoolslevel > select > option").eq(0).html("Team Member Level");
            $("#btnClear").hide();
        }
        ...
        if ((a$.urlprefix() == "cox.") || (a$.urlprefix() == "cox-mnt.") || (a$.urlprefix().indexOf("cox-NOTmake40") >= 0) || (a$.urlprefix() == "v3.")) {
            $(".locname").html("Partner");
            $(".grpname").html("Location"); // really?
            csrranking = "PartnerTier";
            if (a$.exists(controlopts)) {
        ...
        if ((a$.urlprefix() == "chime.")) {
            ...
            $("#StgReportScoreModelID .stg-steps > span").eq(0).html("Chime");
            $("#StgReportScoreModelID select > option").eq(0).html("Chime Score");
            $("#ReportScoreModel select > option").eq(0).html("Chime Score");
            //$(".app-header").removeClass("gradient-lightest"); //In new theming, this messes it up.
    ...
    function formatraw(raw, fmt) {
        ...
        if ((!exists(fmt)) || (fmt == "")) {
            ...
            if (fmt == "hh:mm:ss") {
                ...
                if (fmt.indexOf("m.") == 0) raw = raw / 60.0;
                if (fmt.indexOf(".") >= 0) {
    ...
    function bubblerank(needlecall) {

        if (needlecall) $(".mqfrank").hide();

        //MAKEDEV
        //Chime Quarterly Score - This is a clugue that will be handled differently in V3, but I'm using the hole that's used for Team Rank in Cox
        if (false) { //($.cookie("TP1Role") == "Admin") {
            if ((a$.urlprefix() == "chime.")) {
                var csr = $("#selCSRs").val().toString();
                if ((csr != "") && (csr.indexOf(",") < 0) && (csr != "each")) {
                    a$.ajax({
                        type: "GET",
                        service: "JScript",
                        async: true,
                        data: {
                            lib: "pay",
                            cmd: "getcsrquarterscore", //Get Quarter Score, also get bonus eligibility (this code is scheduled for upgrade anyway).
                            CSR: csr,
                            StartDate: $("#selPayperiods").val().split(",")[0],
                            EndDate: $("#selPayperiods").val().split(",")[1]
                        },
                        dataType: "json",
                        cache: false,
                        error: a$.ajaxerror,
                        success: function (json) {
                            if (a$.jsonerror(json)) { } else {
                                var bld = 'Balanced Score for Quarter: <span style="font-size:14px;">' + json.score.toFixed(2) + "</span>";
                                bld += "<br />" + json.datetext;
                                if (json.bonusEligible == "Not Eligible") {
                                    bld = bld + '<br /><br />Agent is currently <span style="color:blue">Not Bonus-Eligible</span> due to Missing KPIs';
                                }
                                else if ((json.bonusEligible == "") || (json.bonusEligible == "N/A")) {
                                    //No display
                                }
                                else if ((json.bonusEligible == "Eligible")) {
                                    bld = bld + '<br /><br />Bonus Eligibility: <span>' + json.bonusEligible + '</span>';
                                }
                                else { //Some type of Ineligible text.
                                    //bld = bld + '<br /><br />Agent is currently <span style="color:blue">Not Bonus-Eligible:</span> ' + json.bonusEligible;
                                    bld = bld + '<br /><br />Bonus Eligibility: <span style="color:blue">' + json.bonusEligible + '</span>';
                                }
        ...
        if (true) { // (!(a$.urlprefix(true).indexOf("mnt") == 0)) { //Only get these live, they get in the way of debugging. ($.cookie("TP1Role") == "Admin") { //DEBUG DEBUG (STOPPED THIS TEMPORARILY)
            if ((a$.urlprefix() == "cox.")) {
                var csr = $("#selCSRs").val().toString();
                if ((csr != "") && (csr.indexOf(",") < 0) && (csr != "each")) {
                    //2022-01-24 - Hijacked for use with collective-solution production hours (same .shiftbid box).
                    a$.ajax({
                        type: "GET",
                        service: "JScript",
                        async: true,
                        data: {
                            lib: "pay",
                            cmd: "getcsrshiftbid", //Get Shift Bid (this code is scheduled for upgrade anyway).
                            CSR: csr,
                            StartDate: $("#selPayperiods").val().split(",")[0],
                            EndDate: $("#selPayperiods").val().split(",")[1]
                        },
                        ...
                        success: function (json) {
                            if (a$.jsonerror(json)) { } else {
                                var bld = json.shiftbidtext;
                                $(".shiftbid").html(bld).show();
                            }
                        }
                    });
                }
            }
            else if ((a$.urlprefix() == "collective-solution.") || (a$.urlprefix() == "da.")) { //Since this uses scopes, I'll call getreport and retrieve the value.
                //duck
                //TODO:  If there are any commas or eaches in the filters, then don't send.
                //var foundeach = false;
                //if ($("#selCSRs").val()) if 
                if (needlecall) {
                    displaytype = "Grid";
                    a$.ajax({
                        ...
                        data: {
                            ...
                            grouping: $("#StgReportGrouping select").val(),
                            scoremodel: $("#StgReportScoreModelID select").val(),
                            cid: "FilterDisplay1", //"COLSOL.ProductionHours",
                            dashboard: $("#StgDashboard select").val(),
                            displaytype: displaytype,
                            context: displaytype
                        },
                        ...
                        cache: false,
                        error: a$.ajaxerror,
                        params: viewparams(0, false).toString().replace("each", "") //Moves the scope up, but for the case of each it probably should move up (also wipes out KPI each).
                        + ((document.getElementById("StgInTraining") != null) ? "&InTraining=" + $("#StgInTraining select").val() : "")
                        + ((document.getElementById("StgAgentStatus") != null) ? "&AgentStatus=" + $("#StgAgentStatus select").val() : "")
                        + ((document.getElementById("StgDisplayScreen") != null) ? "&DisplayScreen=" + $("#StgDisplayScreen select").val() : "")
                        + ((document.getElementById("StgScorecard") != null) ? "&scorecard=" + $("#StgScorecard select").val() : ""),
                        success: function (json) {
                            if (a$.jsonerror(json)) { } else {
                                //TODO: Fishing it out of the html.  If you continue this route you may want to return it in json format (which will be made with the 
                                //var bld = json.report.panel[0].html.substring(json.report.panel[0].html.indexOf("<td>") + 4);
                                //bld = bld.substring(0, bld.indexOf("<"));
                                var bld = "";
                                try {
                                    var bld = json.json.rows[0]["Value"];
                                    bld = bld.replace(/\"/g, '');
                                }
                                catch (e) {
                                }
                    ...
                    if ((a$.urlprefix() == "da.") || (a$.urlprefix() == "ultracx.")) {
                        a$.ajax({
                            type: "GET",
                            service: "JScript",
                            async: true,
                            data: {
                                ...
                                grouping: $("#StgReportGrouping select").val(),
                                scoremodel: $("#StgReportScoreModelID select").val(),
                                cid: "FilterRankGauge", //"COLSOL.ProductionHours",
                                dashboard: $("#StgDashboard select").val(),
                                displaytype: displaytype,
                                context: displaytype
                            },
                                ...
                                if (a$.jsonerror(json)) { } else {
                                    ...
                                    try {
                                        if (json.json.rows.length > 0) {
                                            //Convert to "fields" (CustomChart convention)
                                            var fields = [];
                                            for (var key in json.json.rows[0]) {
                                                fields.push({ name: key, value: json.json.rows[0][key] });
                                            }
                                            bld = appApmContentTriggers.BuildRankGauge(1, fields);
                                            if (bld.indexOf("svg") > 0) {
                                                $(".filterrankgauge").html(bld).show();
                                            }
                                            else {
                                                $(".filterrankgauge").hide();
                                            }
                                        }
                                        else {
                                            $(".filterrankgauge").hide();
            ...
            //TODO: Blow in the link (to ".shiftbid") here.
            $(".shiftbid").html('Click <a style="background-color:white;padding:3px;" href="" class="shiftbid-fiddle">here</a> to edit filter display 1<br/>View results in the live site.').show();
            $(".shiftbid-fiddle").unbind("click").bind("click", function () {
                $("#sandboxlabel").trigger("click");
                setTimeout(function () {
                    var myframe = $("#ReportSandboxIframe").eq(0);
                    if (a$.urlprefix(true).indexOf("mnt") == 0) {
                        //if ($(myframe).attr("src") == "") {
                        $(myframe).attr("src", a$.debugPrefix() + "/3/ng/ReportEditor/default.aspx?prefix=" + a$.urlprefix().split(".")[0] + "&filtercid=FilterDisplay1");
                        //}
                    }
        ...
        if (a$.urlprefix() == "chime.") {
            $(".chime-game").hide();
            $(".chime-attendance-challenge").hide();
            var csr = $("#selCSRs").val().toString();
            if ((csr != "") && (csr.indexOf(",") < 0) && (csr != "each")) {
                // Attendance Challenge
                var DateDiff = { //TODO: Move to applib
                ...
                a$.ajax({
                    ...
                    data: {
                        lib: "pay",
                        cmd: "getMQFRanks", //Get frost-arnett - specific $ Collected Rankings (this code is scheduled for upgrade anyway).
                        CSR: csr,
                        flag: "chimeattendancechallenge",
                        mqf: kpival,
                        StartDate: $("#selPayperiods").val().split(",")[0],
                        EndDate: $("#selPayperiods").val().split(",")[1]
                    },
                    ...
                    cache: false,
                    error: a$.ajaxerror,
                    success: chimeattendancechallengeloaded
                });
                function chimeattendancechallengeloaded(json) {
                    if (json.msg != "" || json.value == "") {
                        $(".chime-attendance-challenge").hide();
                    } else {
                        var bld = "";
                        bld += "Sept. 1 - Dec. 1<br />Attendance Challenge: ";
                        var dend = new Date("December, 1, 2020");
                        var d = new Date();
                        bld += "<b>" + (DateDiff.inDays(d, dend) + 1) + " days left</b>";
                        var sp = json.value.split("/");
                        bld += '<div style="margin-top:6px;margin-left:45px;height:30px;width:180px;border-radius:5px;position:relative;font-weight:bold;"><div style="position:absolute;top:0px;left:0px;height:30px;line-height:30px;width:120px;background-color:white;color:black;">' + sp[0].replace(".00", "").replace(".50", ".5") + ' hrs absent</div><div style="position:absolute;top:0px;left:120px;height:30px;line-height:30px;width:60px;background-color:' + ((sp[1] == "Disqualified") ? 'darkred' : 'green') + ';">' + ((sp[1] == "Disqualified") ? '$0' : sp[1]) + '</div></div>';
                        $(".chime-attendance-challenge").html(bld);
                        //DON'T SHOW CHIME-ATTENDANCE-CHALLENG (leave in place for a while though)E: $(".chime-attendance-challenge").show();
                    }
                }
                ...
                a$.ajax({
                    ...
                    cache: false,
                    error: a$.ajaxerror,
                    success: chimegameloaded
                });
                function chimegameloaded(json) {
                    if (json.msg != "" || json.hide == "hide") {
                        $(".chime-game").hide().unbind();
                    } else {
                        var spl;
                        var bld = "";
                        ...
                        */
                        if (exists(json.rank.daterange)) {
                            $(".chime-game-daterange").html(json.rank.daterange);
                        }
                        if (a$.exists(json.game)) {
                            $(".chime-game .header-rank-label").html(json.game.label);
                            if (json.game.driver == "ChimeElite") {
                                $(".chime-game-project-label").html("Project");
                            }
                            else if (json.game.driver == "CChimeBill2Pay") {
                                $(".chime-game-project-label").html("Global");
                            }
                        }
                        if (json.rank.Team != "") {
                            spl = json.rank.Team.split("/");
                            bld = "";
                            if (spl.length > 4) {
                                bld += '<span>' + spl[4] + '</span>';
                            }
                            else {
                                bld += '<span>' + spl[1] + '</span> of ' + spl[2];
                            }
                            $(".chime-game-team").html(bld).parent().show();
                            bubblefound = true;
                        }
                        else {
                            $(".chime-game-team").parent().hide();
                        }
                        if (json.rank.Location != "") {
                            spl = json.rank.Location.split("/");
                            bld = "";
                            if (spl.length > 4) {
                                bld += '<span>' + spl[4] + '</span>';
                            }
                            ...
                            $(".chime-game-location").html(bld).parent().show();
                            bubblefound = true;
                        }
                        else {
                            $(".chime-game-location").parent().hide();
                        }
                        if (json.rank.Project != "") {
                            spl = json.rank.Project.split("/");
                            bld = "";
                            if (spl.length > 4) {
                                bld += '<span>' + spl[4] + '</span>';
                            }
                            ...
                            $(".chime-game-project").html(bld).parent().show();
                            bubblefound = true;
                        }
                        else {
                            $(".chime-game-project").parent().hide();
                        }
                        $(".chime-game").show().unbind().bind("click", function () {
                            return false;
                        });
                        if (bubblefound) {
                            $(".chime-game-rank-label-found").show();
                        }
                        else {
                            $(".chime-game-rank-label-found").hide();
                        }
                    }
                    if (false) {
                        $(".chime-game").unbind().bind("click", function () {
                            var frameclass = ".acuity-chime-game-framebox";
                            if (!$(".acuity-chime-game-framebox").length) {
                                var bld = "";
                                bld += '<div class=".acuity-chime-game-framebox">';
                                bld += '<div class="qa-wrapper2-popup" style="width:650px;padding-top:20px;border:1px solid black;">';
                                bld += '<a href="" class="cg-close" style="position:absolute;top:10px;right:10px;">X</a>';
                                bld += '<div class="qa-box-popup" style="height:600px;width:600px !important;">';

                                // Content Section
                                bld += '<p><iframe id="ChimeGameIframe" src="../3/ChimeGame.aspx?prefix=' + a$.urlprefix().split(".")[0] + '"  style="border:0px; width: 100%;  height: 100%" ></iframe>';
                                // end Content Section

                                bld += '</div></div></div>';
                                $("body").append(bld);
                                $(".acuity-chime-game-framebox").draggable();
                            }

                            $(".acuity-chime-game-framebox").show();

                            $(".cg-close").unbind().bind("click", function () {
                                $(".acuity-chime-game-framebox").hide();
                                return false;
                            });
                            return false;
                        });
        ...
        if (needlecall) { //($.cookie("TP1Role") == "Admin") {
            if ((a$.urlprefix() == "frost-arnett.") || (a$.urlprefix() == "da.") || (a$.urlprefix() == "ultracx.") || (a$.urlprefix() == "veyo.") || (a$.urlprefix() == "mtm.") || (a$.urlprefix() == "km2.") || (a$.urlprefix() == "test-km2.") || (a$.urlprefix() == "sunpro.") || (a$.urlprefix() == "collective-solution.") || (a$.urlprefix() == "walgreens.") || (a$.urlprefix() == "nspc.") || (a$.urlprefix() == "united.") || (a$.urlprefix() == "united2.")) {
                var csr = "";
                if ($("#selCSRs").val()) {
                    csr = $("#selCSRs").val().toString();
                }
                var kpival;
                var ranktext;
                if ((a$.urlprefix() == "frost-arnett.")) {
                    ranktext = '$ Collected Ranking:';
                    kpival = "5"; //Always $ collected for FA
                }
                else {
                    /*
                    kpival = $("#selKPIs").val();
                    if ((kpival !="") && (kpival !="each") && (kpival.indexOf(",") < 0)) {
                    ranktext = $("#selKPIs").text() + " Ranking:";
                    }
                    else {
                    */
                    ranktext = "Relative Rank:";
                    kpival = "0";
                    //}
                }
                if ((csr != "") && (csr.indexOf(",") < 0) && (csr != "each") && (kpival != "")) {
                    a$.ajax({
                        type: "GET",
                        service: "JScript",
                        async: true,
                        ...
                        error: a$.ajaxerror,
                        success: function (json) {
                            var applyadjustment1 = false;
                            if (json.msg != "") { } else {
                                var spl;
                                var bld = '<div class="ranking-box">';
                                bld += '<div class="ranking-box_headline">';
                                bld += ranktext;
                                bld += '</div>';
                                bld += '<div class="ranking-box_content">';
                                if (json.rank.Team != "") {
                                    spl = json.rank.Team.split("/");
                                    bld += '<div class="ranking-box_stat">';
                                    bld += '<div class="ranking-box_circle">';
                                    if (spl.length > 4) {
                                        bld += '<span style="margin-top:25px;">' + spl[4] + '</span>';
                                    }
                                    else {
                                        bld += '<span>' + spl[1] + '</span> of ' + spl[2];
                                    }
                                ...
                                if (a$.exists(json.rank.Group) && (json.rank.Group != "")) {
                                    spl = json.rank.Group.split("/");
                                    bld += '<div class="ranking-box_stat">';
                                    bld += '<div class="ranking-box_circle">';
                                    if (spl.length > 4) {
                                        bld += '<span style="margin-top:25px;">' + spl[4] + '</span>';
                                ...
                                if (a$.exists(json.rank.Location) && (json.rank.Location != "")) {
                                    spl = json.rank.Location.split("/");
                                    bld += '<div class="ranking-box_stat">';
                                    bld += '<div class="ranking-box_circle">';
                                    if (spl.length > 4) {
                                ...
                                else if (a$.urlprefix() == "collective-solution.") { //Adjust styling for 2 bubbles (not general at all)
                                    applyadjustment1 = true;
                                }
                                if (json.rank.Project != "") {
                                    spl = json.rank.Project.split("/");
                                    bld += '<div class="ranking-box_stat">';
                                    ...
                                    if ((spl[0].toLowerCase() == "project") && ((a$.urlprefix() == "united.") || (a$.urlprefix() == "united2."))) {
                                        bld += '</div>GLOBAL</div>';
                                    }
                                    else if ((spl[0].toLowerCase() == "project") && (a$.urlprefix() == "walgreens.")) {
                                        bld += '</div>LOB</div>';
                                    }
                                    else {
                                        bld += '</div>' + spl[0] + '</div>';
                                ...
                                bld += '<div class="clearfix"></div></div></div>';
                                $(".mqfrank").html(bld).show();
                                if (applyadjustment1) {
                                    $(".ranking-box_content").css("padding-left", "45px");
                                    $(".ranking-box_stat").css("margin-left", "10px");
                                }
                ...
                else if ((a$.urlprefix() == "da.") || (a$.urlprefix() == "ultracx.")) { //Higher scoped rank bubbles (not CSR, that was handled above for now).
                    var scope = "";
                    var scopeval = "";

                    if (true) { //($("#selCSRs").val()) {
                        if ($("#selCSRs").val().toString() != "") {
                            scope = "csr";
                            scopeval = $("#selCSRs").val().toString();
                        }
                        else if ($("#selTeams").val().toString() != "") {
                            scope = "team";
                            scopeval = $("#selTeams").val().toString();
                        }
                        else if ($("#selGroups").val().toString() != "") {
                            scope = "group";
                            scopeval = $("#selGroups").val().toString();
                        }
                        else if ($("#selLocations").val().toString() != "") {
                            scope = "location";
                            scopeval = $("#selLocations").val().toString();
                        }
                        else if ($("#selProjects").val().toString() != "") {
                            scope = "project";
                            scopeval = $("#selProjects").val().toString();
                        }
                    }
                    if (((scope == "team") || (scope == "group")) && (scopeval.indexOf(",") < 0) && (scopeval != "each") && (kpival != "")) {
                        a$.ajax({
                            type: "GET",
                            service: "JScript",
                            async: true,
                            data: {
                                lib: "pay",
                                cmd: "getMQFRanks", //Get frost-arnett - specific $ Collected Rankings (this code is scheduled for upgrade anyway).
                                CSR: "", //Signal to use scope/scopeval
                                scope: scope,
                                scopeval: scopeval,
                                mqf: kpival,
                                StartDate: $("#selPayperiods").val().split(",")[0],
                            ...
                            success: function (json) {
                                ...
                                if (json.msg != "") { } else {
                                    ...
                                    bld += '<div class="clearfix"></div></div></div>';
                                    $(".mqfrank").html(bld).show();
                                    if (applyadjustment == 1) {
                                        $(".ranking-box_content").css("padding-left", "45px");
                                        $(".ranking-box_stat").css("margin-left", "10px");
                                    }
                                    else if (applyadjustment == 2) {
                                        $(".ranking-box_content").css("padding-left", "86px");
                                        $(".ranking-box_stat").css("margin-left", "10px");
                                    }
                                }
                            }
            ...
            if (a$.urlprefix() == "sunpro.") { //TODO: Replace with content block.
                var csr = $("#selCSRs").val().toString();
                var team = $("#selTeams").val().toString();
                var group = $("#selGroups").val().toString();
                a$.ajax({
                    type: "GET",
                    service: "JScript",
                    async: true,
                    ...
                    success: function (json) {
                        if (json.msg != "") { } else {
                            var spl;
                            if (json.found) {
                                console.log("debug:WinInfo=" + json.WinInfo);
                                console.log("debug:ConversionInfo=" + json.ConversionInfo);
                                $(".stats-blocks").show();
                                var ws = json.WinInfo.split("|");
                                var cs = json.ConversionInfo.split("|");
                                $(".conversion-stat_total").html(cs[0]); //AppointmentCount
                                $(".conversion-stat_streak").html(cs[3]); //Days in Streak
                                $(".conversion-stat_goal").html(cs[2]); //GoalPct (formatted with %)
                                $(".wins-rank-item_wins").html(ws[0]); //WinCount
                                $(".wins-rank-item_rank").html(ws[1] + " <span>OF " + ws[2] + "</span>"); //UserRank of TotalUsers
                                $(".wins-rank-item_winrate").html(ws[4]); //WinPct (formatted with % at end)
                                $(".wins-second-rank").html(ws[5] + " <span>OF " + ws[2] + "</span>"); //WinRank of TotalUsers
                            }
                            else {
                                $(".stats-blocks").hide();
                            }
    ...
    function formatrank(rank, needlecall) {
        var ret = "";

        $(".teamrank").hide(); //TODO: add needlecall (this is called on every iteration).
        $(".shiftbid").hide(); //TODO: add needlecall (this is called on every iteration).

        bubblerank(needlecall);
        ...
        if ($("#StgRanking select").val() == "On") {
            ...
            $("#rank1pin1").css("left", "-100px");
            $("#rank1pin2").css("left", "-100px");
            $("#rank2pin1").css("left", "-100px");
            $("#rank2pin2").css("left", "-100px");
            ...
            if (csr == "") {
                ...
                if (rank == "") {
                    ...
                    if (parseInt(rs[2]) > 0) {
                        ...
                        $("#rank1pin1").css("left", ((controlopts.rankopts.xoffset - 10) + (controlopts.rankopts.barwidth * pct)) + "px");
                        $("#rank1pin2").css("left", ((controlopts.rankopts.xoffset - 10) + (controlopts.rankopts.barwidth * pct)) + "px");
                        $("#rank2pin1").css("left", ((controlopts.rankopts.xoffset - 10) + (controlopts.rankopts.barwidth * pct)) + "px");
                        $("#rank2pin2").css("left", ((controlopts.rankopts.xoffset - 10) + (controlopts.rankopts.barwidth * pct)) + "px");
                        /*
                    ...
                    if (rs.length > 4) {
                        ret = rs[4]; //Display a string instead of the ranking
                    }
                    else {
                        if (rs[1] <= 0) {
                            ret = "Rank in " + rs[0] + ": Not Ranked";
                        }
                        else {
                            ret = "Rank in " + rs[0] + ": " + pt + rs[1] + " of " + rs[2];
                        }
                    }
    ...
    function connectionfilter(str) {
        ...
        } else if (str == "Sidekick") {
            return "Operations&Model=4";
        }
        return str;
    }
    ...
    window.appApmDashboard = {
        ...
        setAddBoxList: setAddBoxList,
        getSuppressBoxList: getSuppressBoxList,
        getAddBoxList: getAddBoxList,
    ...
    ko.postbox.subscribe("FILTER_PARAMS", function (newValue) {
        ...
        $("#dashboard_filterbuild").html(filterParams);
    });
