(function () {
    ...
    function multiChoiceQuiz(ostr) {
        ...
        if (a$.exists(o.libid) && a$.exists(o.backstopid)) {
            a$.ajax({ type: "POST", service: "JScript", async: false, //THIS IS NOT ASYNC
                data: {
                    lib: "qa",
                    cmd: "findLibraryFirstUnanswered",
                    database: "C",
                    formid: 396, //TODO: Generalize formid
                    qstbackstopid: o.backstopid,
                    qstlibid: o.libid,
                    examinee: $.cookie("TP1Username")  //If ommitted, it will compare entby to logged in user.
                },
                dataType: "json",
                cache: false,
                error: a$.ajaxerror,
                success: function (json) {
                    if (json.unansweredFound) {
                        libid = json.unansweredid;
                    }
                    else {
                        libid = o.libid;
                    }
        ...
        $(".qa-multichoice-quiz").hide();
        a$.ajax({ type: "POST", service: "JScript", async: true,
            data: {
                lib: "qa",
                cmd: "checkLibraryDuplicates",
                database: "C",
                formid: 396, //TODO: Generalize formid
                qstlibid: libid,
                examinee: $.cookie("TP1Username")  //If ommitted, it will compare entby to logged in user.
            ...
            success: function (json) {
                if (a$.jsonerror(json)) {
                    ...
                    if (json.dupFound) {
                        //alert("debug:Question Already Answered");
                    }
                    else {
                        function delayshow() {
                            $(".qa-multichoice-quiz iframe").attr("src", "/jq/monitors/V3/Quiz_base_1.aspx?formid=396&masterid=&prefix=walgreens&libid=" + libid); //TODO: Generalize iframe src.
                            $(".qa-multichoice-quiz").show();
                            $(".qa-multichoice-quiz .qa-close").bind("click", function () {
                                $(".qa-multichoice-quiz").hide();
                                $(".quiz-open-link").html("Re-display Quiz").unbind().bind("click", function () {
                                    $(".qa-multichoice-quiz").show();
                                    $(this).html("");
                                });
                            });
                        }
                        setTimeout(delayshow, 15000);
                    }
                }
            }
        });
