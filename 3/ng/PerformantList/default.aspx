...
<ng-legacy-container>
  ...
  <section class="acuity-app" ng-show="LOGGED_IN()">
        ...
        <div class="container" style="overflow-y:scroll;height:calc(100%)">
            <div>
                <a>
                    <h2>List:</h2>
                    <select ng-model="selectedlistid">
                        <option ng-repeat="list in json.lists" value="{{ list.id }}">{{ list.desc }}</option>
                    </select> 
                    ...
                    <ul style="height:512px;width:512px;border:1px solid black;overflow:auto;overflow-x:hidden;list-style-type:none;margin:0;padding:0">
                        ...
                        <li style="width:auto;position:relative" ng-repeat="letter in json.letters" ng-show="selectedlistid==letter.idlist&&(filterchecked(letter.letterid)||filterchecked(letter.lettername))&&letter.lettername!=''">
                            <p style="position:relative;left:10px">{{ letter.letterid }}</p>
                            <p style="position:relative;left:10px">{{ letter.lettername }}</p>
                            <div style="position:absolute;top:-5px;right:10px"><button ng-click="removeletter($index)">DELETE</button></div>
                            <hr>
                        </li>
                    </ul>
                    <hr />
                    <div>
                    <h2>Letter ID:</h2>
                    <input type="text" style="width:300px" ng-model="newletterid">
                    <h2>Name:</h2>
                    <input type="text" style="width:300px" ng-model="newlettername">
                    </div>
                    <button style="position:relative;left:400px;top:-80px;width:100px;height:50px;z-index:3" ng-click="addletter()">ADD LETTER</button>
                </a>
            </div>
