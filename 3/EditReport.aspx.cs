...
public partial class three_EditReport : SitePage {
...
//            Session("TP1Role") = Request.QueryString("role").ToString
        ...
        {
            String par = "";
            if (HttpContext.Current.Request.Url.AbsoluteUri.ToString().IndexOf('?') >= 0)
            {
                par = "|" + HttpContext.Current.Request.Url.AbsoluteUri.ToString().Substring(HttpContext.Current.Request.Url.AbsoluteUri.ToString().IndexOf('?')).Replace("&", "~");
            }            
            String redirect = "~/applib/dev/LOGIN1/LOGIN1-view/loginNew.aspx?url=../../../../3/EditReport.aspx" + par;
