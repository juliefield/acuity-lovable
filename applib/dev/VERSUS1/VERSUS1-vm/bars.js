	$(document).ready(function() {
        var index=0;
        var lock=false;
        document.getElementById("name1").innerHTML=versus[0].person.firstnm+" "+versus[0].person.lastnm;
        document.getElementById("name2").innerHTML=versus[1].person.firstnm+" "+versus[1].person.lastnm;
        ...
        function bars(){
            lock=true;
            setTimeout(function () {
                $(".track").append("<div class=bar1 style=left:"+start1[index]*100+"%;background-color:"+colorlist[index]+" id=bar"+barnumA+"></div>");
                $("#bar"+barnumA).animate({
                width: realdist1[index]*100+"%"
                }, {duration: 30000/daynum,
                    easing: "linear"
                });
                $(".track").append("<div class=bar2 style=left:"+start2[index]*100+"%;background-color:"+colorlist[index]+" id=bar"+barnumB+"></div>")
                $("#bar"+barnumB).animate({ 
                ...
                document.getElementById("announcer").innerHTML=announcements[index];
                $(".button").remove();
                $(".board").append("<button class=button id=button onclick=reload()>Reset</button>");
                },firstcheck)
                if(index!=0)firstcheck=30000/daynum;
                if(index<scorelist1.length) {
                bars();
                }
            }, firstcheck)
