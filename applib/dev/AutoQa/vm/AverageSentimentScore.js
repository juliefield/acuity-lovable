angularApp.directive("ngAverageSentimentScore", ['api', '$rootScope', function (api, $rootScope) {
   return {
      templateUrl: a$.debugPrefix() + '/applib/dev/AutoQa/view/AverageSentimentScore.htm?' + Date.now(),
      scope: {
         assoc: "@",
         text: "@",
         details: "@",
      ...
      link: function (scope, element, attrs, legacyContainer) {
         ...
         function LoadDirective() {
            HideAll();
            LoadAverageSentimentScore();
         }
         function LoadAverageSentimentScore(callback)
         {
            GetAverageSentimentScore(function(){
               RenderAverageSentimentScore(callback);
            });
         }
         /* Data Loading END */
         /* Data Pulls START */
         function GetAverageSentimentScore(callback) {
            if (callback != null) {
               callback();
            }
         }
         ...
         function RenderAverageSentimentScore(callback)
         {
            let averageSentimentScore = null;
            $("#averageSentimentScore", element).empty();
            ...
            $("#averageSentimentScore", element).append(averageSentimentScore);
            if(callback != null)
            {
               callback();
            }
