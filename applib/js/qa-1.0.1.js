                        ...
                        if (userstates[i].assignment.type == "login") {
                            if (userstates[i].assignment.delivery.notification == "contextual") {
                                for (var c in userstates[i].assignment.delivery.contexts) {
                                    if (passesFilterCritera(userstates[i].assignment.delivery.contexts[c].filters)) {
                                        if (userstates[i].assignment.delivery.contexts[c].startup == "popup") {
                                            startupformids.push(userstates[i].formId); //So we can retrieve the outlines momentarily.
                                        }
                                        else if (userstates[i].assignment.delivery.contexts[c].startup == "link") {
                                            //Install the linking mechanism (probably some html that initiates the form).
                                            var h = userstates[i].assignment.delivery.contexts[c].html;
                                            if (a$.exists(userstates[i].assignment.delivery.contexts[c].appendSelector)) {
                                                $(userstates[i].assignment.delivery.contexts[c].appendSelector).append(h);
                                                $(userstates[i].assignment.delivery.contexts[c].appendSelector).children().last().unbind().bind("click", function () {
                                                    userstates[i].assignment.delivery.contexts[c].myselector = this;
                                                    var myformids = [];
