...
(function() {
    ...
    function ajax(a, xd) {
        ...
        a.error = function(xhr, status, error) {
            if (ajaxTrace.active || (poptrace!="")) {
                ...
                //TODO: Note Update 2023-01-01 - Add a robust error handling system.
    ...
    function jsonerror(json)
    {
       ...
       if (json.errormessage) {
         //alert(json.msg);
         $(".err-content",WindowTop().document).html(json.msg);
         $(".err-icon",WindowTop().document).show();
         return true;
       }
    }
    ...
    function addOption(elSel, text, val) {
        var addit = true;
        if (elSel.options.length < 30) { //If list is short, don't allow duplicate entries (needs tested for drag)
            for (var i in elSel.options) {
