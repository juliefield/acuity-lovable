...
(function () {
    ...
    function getmessages() {
        ...
        function loaded(json) {
            ...
            $(".messages-compose input").bind("click", function () {
                ...
                bld += '<span class="message-compose-send"><input type="button" value="Send"/></span>';
                bld += '</div>'
                bld += '<div style="float:left;">';
                bld += '  <div style="margin-top: 10px;float:left;">';
                bld += '    To:&nbsp;';
                bld += '  </div>';
                bld += '  <div class="message-to-manual" style="margin-top: 10px;float:left;">';
                bld += '    <select id="composeto" style="width:500px;" data-placeholder="Click Here to Select Recipients.  Change the Dashboard Filter for More Choices." class="message-compose-to" multiple><option value=""></option></select>';
                bld += '  </div>';
                ...
                bld += '</div>';
                bld += '<div style="clear:both;"><div class="message-compose-subject">Subject&nbsp;<input type="text" id="composesubject" /></div></div>';
                //MOD 10/14/2015
                //WAS:
                //bld += '<div><textarea type="text" id="composebody" class="message-compose-body"/></textarea></div>';
                //IS:
                bld += '<div><div id="composebody" contenteditable="true" class="message-div-compose-body"/></textarea></div>';

                bld += '<div class="message-compose-send" style="float: left;"><input type="button" value="Send"/>';
                bld += '</div>';
                    ...
                    function loaded(json) {
                        if (a$.jsonerror(json)) {
                            ...
                            if (foundsent) {
                                ...
                                m.from = "";
                                m.to = tostr;
                                m.subject = $.trim($("#composesubject").val());
                                m.body = $.trim($("#composebody").html());
                                m.read = "Y";
                                m.labels = new Array();
    ...
    */
    ...
    function initchatwindow() {
        ...
        var bld = '<div class="chat-window">';

        bld += '<div class="chat-label">Chat / Meet';

        if (false) { //($.cookie("TP1Role") == "Admin") {
            bld += '<span class="chat-plus">+</span><span class="chat-plus-team">+tm</span>';
            chat20ui = true;
        }
        else {
            switch ($.cookie("TP1Username").toLowerCase()) {
                ...
                case "dweather":
                default:
                    bld += '<span class="chat-plus-team">+tm</span>';
                    chat20ui = true;
            }
        }

        bld += '<div class="poll-suspend"></div><span class="chat-stats"></span><div class="chat-select-all">all</div><div class="chat-save">Save</div><div class="chat-mute"><input type="checkbox" /><label>Mute</label></div><div class="chat-refresh" href="#"><i class="fa fa-sync-alt"></i></div><a class="window-close chat-window-close" href="#" title="Close Chat Box"><i class="fa fa-times"></i></a>';
        bld += '</div>';
        ...
        for (var i = numsessions - 1; i >= 0; i--) {
            if (i > 0) {
                ...
                bld += '<div class="chat-session-close"><i class="fa fa-times"></i></div>';
            }
            else {
            ...
            bld += '<div class="chat-chosenbox"><select data-placeholder="Chat with..." class="chat-parties" id="chatparties" multiple></select></div></div>';
        ...
        bld += '<div class="chat-send" style="width: 400px;">' + $.cookie("TP1Username") + ':&nbsp;<input class="chat-inputline" type="text"/>';
    ...
    function clickedchatbubble() {
        ...
        $(".chat-window").show().css("width", (w * 315) + "px");
        $('.chat-area').scrollTop($('.chat-area')[0].scrollHeight); //review
        $("#inputline").focus();
    }
                        ...
                        */
                        ...
                        if ($(me).attr("id") != "chatparties") {
                            if ($(" option", this).length == 0) {
                                $(me).append($("<option></option>").attr("value", "").text("Chat with..."));
                            }
                        }
                        ...
                        for (var i in json.chatees) {
                            ...
                            if (json.chatees[i].uid != "") {
                                ...
                                if ((a$.urlprefix().indexOf("make40.") >= 0) || (a$.urlprefix() == "sprintgame.")) { //duck
                                if ($.cookie("TP1Role") == "CSR") {
    ...
    function bldChatWindow(o) {
        ...
        if (!a$.exists(o.chatWith)) {
            bld += '<div class="chat-chosenbox"><select data-placeholder="Chat with..." class="chat-parties">';
            /*
    ...
    function addsingle(csr, csrtext) {
        ...
        if (!found) {
            ...
            numsessions += 1;
            clickedchatbubble();
            var bld = bldChatWindow({ chatWith: { text: csrtext, val: csr, online: online} });
            $(".chat-sessions").prepend(bld);
            //chatlistinit(false);
            bindchatsession(true);
        }
