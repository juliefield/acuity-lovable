...
(function () {
    ...
    function commoninit(pointsbalance) {
        ...
        */
        ...
        function getledger(location) {
            if (a$.urlprefix() == "ers.") {
                $(".treasurehunt-ers").show();
                $("#tabs-13").addClass("ers-th_bg");
            }
            else if ((a$.urlprefix() == "united.") || (a$.urlprefix() == "united2.")) {
                $(".treasurehunt-united").show();
                $("#tabs-13").addClass("united-th_bg");

                var scsr = $("#selCSRs option:selected").text().split(",");

                $(".united-th_agent-name").html(scsr[1] + " " + scsr[0]);
            }
            pdx = 0;
            if (location == 9) pdx = 1;
            var csr;
            ...
            function loaded(json) {
                if (a$.jsonerror(json)) { } else {
                    ...
                    var redeemInj = '<p>You currently have a total of <b><span class="CEPointsBalance"></span></b> <span class="labPointsLower">points</span> <span class="CEPointsInstructions"></span></p><span class="labPointsUpper">Points</span> to Redeem: <input id="CEPointsRedeemAmount" type="text" style="width:50px;" disabled="disabled" value="' + prizes[pdx].table[0].points + '" /> <select id="CEPointsRedemptionType"><option value="">--select redemption type--</option><option value="Cash">Cash</option><option value="Prize">Prize</option></select> <input id="CEPointsRedeem" type="button" disabled="disabled" value="Redeem" />';
                    var pb = 0;
                    var cb = 0;
                    for (var i in json.ledger) {
                        if (json.ledger[i].paytype.toLowerCase() == "rankcoupons") {
                            cb += json.ledger[i].amount;
                        }
                        else {
                    ...
                    if (true) { //(stgRankPointsLabel == gemTheme) {
                    ...
                    if ((a$.urlprefix() == "united.") || (a$.urlprefix() == "united2.")) {
                        $(".united-th_PointsBalance").html(pb);
                        $(".united-th_CouponsBalance").html(cb);
                    }
    ...
    function mgrinit() {
        ...
        function getmgrledger() {
            ...
            if (($.cookie("TP1Username") == "syanez") || ($.cookie("TP1Username") == "mark.vander")) {
                pdx = 1;
            } else {
                pdx = 0;
            }
        ...
        table: [{
            ...
            Prize: "Tablet ($225 Value)"
                ...
                },
                {
                    points: 2500,
                    Cash: "$1,200",
                    Prize: "No other option."//"Carnival Cruise Gift Card ($1,500 Value)<br />Expires October 31, 2016"
                },
                {
                    points: 3000,
                    Cash: "$1,475",
                ...
                },
                {
                    points: 6000,
                    Cash: "$3,300",
                    Prize: "No other option."//"7 Day Hawaiian Resort Stay and 1 round trip<br />airline ticket ($3,500 Value)<br/>Expires December 31, 2016"
                }
            ]
    },
        { //Guatemala
