...
public class jshandler implements IHttpHandler{
    ...
    function IHttpHandler.ProcessRequest(context : HttpContext){
        ...
        if (context.Request.QueryString.Count > 0) {
            ...
            token = context.Request.QueryString["acuitytoken"];
        }
        else {
            //Assume it's a POST
            wh.QueryString = context.Request.Form;
            token = context.Request.Form["acuitytoken"];
        }
        ...
        if (!cachefound) {
            var obj;
            if (token != "F7D9DFCB832B77EBF91A256823999") {
                obj = { msg: "Token not found or invalid." };
            }
            else {
              obj = wh.PreProcess();
            }
